/**
 * Stop one live paper sniper parent.
...
export type OmsPaperSniperStopRefuseReason =
  | 'missing_parent'
  | 'not_running'
  | 'already_stopped'
  | 'not_live'
...
export type OmsPaperSniperStopRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperSniperStopRefuseReason;
  readonly detail: string;
};

export type OmsPaperSniperStopOk = {
  readonly ok: true;
...
export type OmsPaperSniperStopResult =
  | OmsPaperSniperStopOk
  | OmsPaperSniperStopRefusal;

function refuse(
  reason: OmsPaperSniperStopRefuseReason,
  detail: string,
): OmsPaperSniperStopRefusal {
...
/**
 * Stop a live paper sniper parent. Existing children stay as recorded.
 * Residual is not released or consumed. Paper off refuses a live venue.
 */
export function stopPaperSniperParent(input: {
  parentClientOrderId?: string;
  kind?: string;
  status?: string;
  remaining?: string | null;
  paper?: PaperGate;
}): OmsPaperSniperStopResult {
