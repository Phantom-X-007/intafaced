...
export function copyIntelMoneyDenyBoardCard(
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): {
...
/** L3 — status line. */
export function copyIntelMoneyDenyStatusLine(
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): string {
...
export function parseCopyIntelMoneyDenyStatusLine(line: string): {
  ...
  const m = line
    .trim()
    .match(/^tools=(\d+) ledger_post=([01]) trade_order=([01]) copy_follow=([01])$/);
  if (!m) return null;
  return {
    tools: Number(m[1]),
...
export function copyIntelMoneyDenyStatusLineMatches(
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): boolean {
...
/** L3 — export line. */
export function copyIntelMoneyDenyExportLine(
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): string {
  const c = copyIntelMoneyDenyBoardCard(tools);
...
/** L3 — full export. */
export function copyIntelMoneyDenyExportText(
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): string {
...
export function isCopyIntelMoneyDenied(
  tool: string,
  tools: readonly string[] = COPY_INTEL_MONEY_WRITE_TOOLS,
): boolean {
