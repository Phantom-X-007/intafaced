...
import './halt-law.js';
import { DELIST_POLICY_MISSING, MISSING_EVIDENCE, PERMISSIONLESS_LISTING, installRulebookRefuse } from './rulebook-refuse.js';
...
type ListDoor = MatchingEngine & {
  halt(
    marketId: string,
    cmd: { operatorId?: string | null; evidence?: string | null; evidenceRefs?: readonly string[] | null },
  ): ReturnType<MatchingEngine['halt']>;
  delist(
    marketId: string,
    cmd: { operatorId?: string | null; policyId?: string | null; policyVersion?: string | null },
  ): ReturnType<MatchingEngine['delist']>;
  listMarket: (cmd: {
    marketId?: string;
    permissionless?: boolean;
    listingPolicy?: string;
    listingAuthority?: string;
  }) => Promise<{ accepted: boolean; listed: boolean; rejected?: { code: string; message: string } }>;
};

function engine(): ListDoor {
...
describe('rulebook refuse — emergency evidence, delist policy, permissionless listing', () => {
  it('halt without evidence refuses — market is not halted; submits still rest', async () => {
    ...
    expect(typeof limitBuy().price).toBe('bigint');
  });
  ...
  it('permissionless listMarket refuses — no invented book', async () => {
    const live = engine();
    const listed = await live.listMarket({ marketId: MARKET, permissionless: true });
  ...
  it('listMarket without listingPolicy refuses — engine does not invent a listing', async () => {
    const live = engine();
    const listed = await live.listMarket({ marketId: MARKET });
    expect(listed.accepted).toBe(false);
    expect(listed.rejected?.code).toBe(PERMISSIONLESS_LISTING);
    expect(live.hasMarket(MARKET)).toBe(false);
    expect(live.existingBook(MARKET)).toBeNull();
  });
});
