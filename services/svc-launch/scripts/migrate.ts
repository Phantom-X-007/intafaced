 ...
 * Migration runner.
 *
 * Applies every `NNNN_*.sql` in order, tracked in `launch.__migrations` so a
 ...
 * anyone should run it against a database holding live raises.
 */
 ...
 * strays outside `launch` fails here rather than in production.
 */
const url = process.env.DATABASE_URL ?? 'postgres://svc_launch:svc_launch@localhost:5433/intafaced';
...
async function main() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS launch.__migrations (
  ...
  if (down) {
    for (const up of [...ups].reverse()) {
      ...
      await sql`DELETE FROM launch.__migrations WHERE name = ${up}`;
    }
    console.log('✓ reversed');
    return;
  }

  const applied = new Set((await sql<Array<{ name: string }>>`SELECT name FROM launch.__migrations`).map((r) => r.name));
  ...
  for (const file of ups) {
    ...
    await sql`INSERT INTO launch.__migrations (name) VALUES (${file})`;
  }
