...
| Sand-castle mill: leftover honesty rebase, sequential i18n pin-only PRs, occupancy spawns (“3 more agents”) with no user-visible Done-bar | Module-Done is infinite; 240 commits / 0 mountains is a failed run                                                    | One tracker mountain; close or cut; see `AGENTS.md`                   |
| Ping Nitro to Approve, to “make audit true”, or to start coding                                                                           | Human wait-loop; Class X / paid audit are not session gates                                                           | Keep shipping; **do not ping Nitro**; leave `audited:true` false      |
| Flip or advertise a fake `audited:true` / marketing “audited”                                                                             | Sale lie; pipeline already refuses a fake badge                                                                       | Honest `audited: false` until a paid external hash+signer matches     |
...
```bash
pnpm gates            # every entry in tooling/ci/gates.mjs (count is live; do not
                      # hardcode it here) — brand §0.7 · custody §16.10 ·
                      ...
                      # agent-autoload · path-collide/wt-gc self-tests · i18n (advisory)
