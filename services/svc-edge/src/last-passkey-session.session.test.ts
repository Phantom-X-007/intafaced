...
describe('last enrolled passkey unenrolled closes the HTTP session door', () => {
  it('last remaining verified cred still places on the existing session', async () => {
  ...
  it('last remaining places, then empty after last unenroll cannot', async () => {
    const token = await accessToken();
    const last = await exchangePrincipal(
      { authorization: `Bearer ${token}` },
      {
        ...options,
        fetch: fetchIdentity({
    ...
    expect(last.rejected).toBeNull();
    expect(last.principal?.userId).toBe(USER);
  ...
  it('last cred without lastVerifiedAt cannot place', async () => {
    const token = await accessToken();
