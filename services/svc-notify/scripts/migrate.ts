 ...
 * Migration runner.
 *
 * Applies every `NNNN_*.sql` in order, tracked in `notify.__migrations` so a
 ...
 * anyone should run it against a database holding real inboxes.
 */
 ...
 * strays outside `notify` fails here rather than in production.
 */
const url = process.env.DATABASE_URL ?? 'postgres://svc_notify:svc_notify@localhost:5433/intafaced';
...
async function main() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS notify.__migrations (
  ...
  if (down) {
    for (const up of [...ups].reverse()) {
      ...
      await sql`DELETE FROM notify.__migrations WHERE name = ${up}`;
    }
    console.log('✓ reversed');
    return;
  }

  const applied = new Set((await sql<Array<{ name: string }>>`SELECT name FROM notify.__migrations`).map((r) => r.name));
  ...
  for (const file of ups) {
    ...
    await sql`INSERT INTO notify.__migrations (name) VALUES (${file})`;
  }
