 ...
 * argument shape. Ledger is not a proxy: there is no upstream table and no
...
async function ledger(): Promise<FastifyInstance> {
  const instance = Fastify({ logger: false });
  registerMetrics(instance, { service: 'svc-ledger' });
...
describe('GET /metrics', () => {
  ...
  it('emits the duration series a scrape of ledger /metrics must contain', async () => {
    const instance = await ledger();
    ...
    expect(count?.labels.service).toBe('svc-ledger');
  ...
  it('counts a local request under _local, method and status class', async () => {
    ...
    expect(health?.labels.service).toBe('svc-ledger');
  ...
  it('labels every path _local rather than inventing a module from the URL', async () => {
    const instance = await ledger();
    await instance.inject({ method: 'GET', url: '/health' });
    await instance.inject({ method: 'GET', url: '/operator/freeze' });
    await instance.inject({ method: 'POST', url: '/trpc/post' });
  ...
  it('does not carry a query string into a label', async () => {
    const instance = await ledger();
    await instance.inject({ method: 'GET', url: '/health?owner=alice&cursor=abc' });
...
describe('moduleLabel', () => {
  it('is always _local — ledger is not a proxy', () => {
    ...
    expect(moduleLabel('/operator/freeze')).toBe(MODULE_LOCAL);
    expect(moduleLabel('/trpc/post')).toBe(MODULE_LOCAL);
    expect(moduleLabel('/api/ledger/balance')).toBe(MODULE_LOCAL);
  });
});
