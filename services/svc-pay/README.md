...
The payments core (Â§6.1). Merchants, the payment lifecycle, settlement into the ledger, and the `RailAdapter` interface every external rail sits behind.

**What it is NOT:** it does not hold balances (the ledger does), it does not choose between rails (smart routing is its own feature), and it does not know the name of a single payment processor. Every rail â€” the two here and every one that comes later â€” is an implementation of one interface, and `src/rails/conformance.ts` is what keeps that true.
...
```
created â”€â”€authorizeâ”€â”€â–¶ authorized â”€â”€captureâ”€â”€â–¶ captured â”€â”€settleâ”€â”€â–¶ settled
   â”‚                        â”‚                      â”‚                   â”‚
   â””â”€â”€â”€â”€â”€â”€â–¶ failed â—€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                      â””â”€â”€refundâ”€â”€â–¶ refunded â—€â”€â”€refundâ”€â”€â”˜
```

`payment_events` is the append-only state history. Every transition appends a row; **nothing is ever overwritten**, and the database enforces that with a trigger rather than trusting the application. `payments.status` is a projection of that log, and the captured and refunded totals are summed from it â€” there is deliberately no `captured_amount` column, because a running total beside an event log is a second source of truth for money.
...
| -------------------- | ------------ | ----------------------------------------------------------------------------- | ------------------------------------------------ |
| `health`             | public       | â€“                                                                             | `{ ok, service, rails }`                         |
| `railHealth`         | `pay:read`   | â€“                                                                             | `RailHealth[]`                                   |
| `merchant.create`    | `pay:write`  | `{ mode, pricing: { feeBps } }` â€” `userId` from the principal, never the body | `{ id, userId, mode, feeBps }`                   |
| `merchant.profile`   | `pay:write`  | `{ merchantId, checkoutConfig, feeRouting, domains }`                         | `{ id, merchantId }`                             |
| `merchant.balances`  | `pay:read`   | `{ merchantId, assetId }`                                                     | `{ clearing, available }` â€” read from the ledger |
...
| `settlement.payout`  | `pay:payout` | `{ settlementId, railId, destination }` â€” dest must match rail (EVM/IBAN)     | `Settlement`                                     |
| `settlement.get`     | `pay:read`   | `{ settlementId }`                                                            | `Settlement`                                     |
| `settlement.release` | `pay:write`  | `{ settlementId, reason }` â€” unstick pending freeze; **no ledger move**       | `Settlement`                                     |
...
Everything above is **merchant** money â€” a third party pays a merchant. These are the other half: a **user's own** balance entering and leaving the book.
...
- **No jurisdiction guard, and no tier check on the payee.** The matrix judges the user being served, and the principal here is an operator who is not the beneficiary â€” checking their tier measures the wrong person. And money that has already reached a rail must always be bookable: refusing to credit an unverified user does not undo their payment, it strands it at the boundary. The gate belongs on what a balance can **do**, not on being allowed to receive one.

#### `withdrawal.create` â€” the interactive path off the platform

- **`trade:withdraw`**, INTERACTIVE_ONLY: no API key may hold it, and `requireScope` refuses a session that has not passed 2FA. A normal session does not carry it at all â€” `auth.stepUp` in svc-identity mints the five-minute token that does.
- **`{ module: 'ledger' }`, not `{ module: 'pay' }`.** The matrix rule governing a user moving their own custodial balance is the rule for the module that **holds** it, and the ledger holds it (`custodial: true`, `OPEN_BASIC`). `pay`'s `full` tier governs merchant acquiring â€” third-party card money â€” a different risk and a different subject. Gating a withdrawal above the tier that admitted the value would build a one-way door: verified enough to deposit and trade, not verified enough to leave.
...
### Which rails are real â€” read this before believing a `sent`

| rail            | `mode`                      | counterparty                                                                                  | what a `payout` returns                                       |
| --------------- | --------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `card-sandbox`  | `sandbox`, permanently      | a `Map` in `card-sandbox.ts`                                                                  | `po_<settlementId>` â€” a string this file invented             |
| `crypto-native` | derived from the chain port | `EvmLiveChain` when configured; else `MemoryChain` (dev) / `UnconfiguredChain` (staging/prod) | a real `txHash` on a live chain; `0xoutâ€¦` / refusal otherwise |
| `bank-payout`   | `absent`, permanently       | nothing â€” sponsor bank / licence is a commercial socket                                       | never succeeds; `pay.rail_not_live` before any hold           |

**Live crypto-native** is wired when all of these are set:

```bash
PAY_CRYPTO_RPC_URL=â€¦          # archive-capable JSON-RPC
PAY_CRYPTO_CHAIN_ID=â€¦
PAY_CRYPTO_DEPOSIT_MNEMONIC=â€¦ # HD acceptance addresses (not the hot wallet)
PAY_CRYPTO_HOT_WALLET_KEY=0xâ€¦ # outbound signing key
PAY_CRYPTO_ASSETS=ETH:native  # or ETH:native,USDT:0xâ€¦:6
```
...
**Why sandbox gates still matter.** A withdrawal settled against a sandbox rail debits the user's real ledger balance, writes a fabricated reference into `withdrawals.rail_ref`, and answers `status: 'sent'`. The books still balance â€” only custody reconciliation can see the lie.
...
2. **Runtime.** `payout` and `refund` re-check at the call site, **before** the ledger moves â€” `withdrawal.create` refuses with `pay.rail_not_live` before a row or a hold exists.
...
- A **production** (or public testnet) RPC, not only compose anvil â€” anvil proves the wiring; it is not a chain decision (`docs/decisions/local-dev-chain.md`).
...
- Live boot wires `PostgresBroadcastStore` (`pay.crypto_broadcasts`, migration 0004 + `signed_raw` in 0012) for multi-replica claimâ†’putSignedâ†’sendRawâ†’put. `MemoryBroadcastStore` remains the unit/local default when no store is passed to `defaultChainFor`. DIRECTION Â§3.1 / D26-P1-P9: the signed raw is journalled **before** `eth_sendRawTransaction`; crash-resume rebroadcasts the identical payload (`claim` â†’ `resume`) instead of signing a second spend. Put-before-receipt still closes the wait-for-inclusion window.
- **Card acquiring** â€” sponsor bank / BIN. Still a Â§13 commercial socket; `card-sandbox` is never it.

`/ready` and `railHealth` both carry `mode` now, because `healthy: true` was accurate and useless â€” a sandbox is reliably healthy at simulating.

> **Interface mismatch, flagged not papered over.** `RailAdapter.payout` takes a `SettlementInstruction`, which is merchant-shaped â€” it is the only payout shape Â§6.1 has. Every adapter uses `settlementId` purely as the payout idempotency key and reads `merchantId` not at all, so a user withdrawal passes its own id and the user id and works correctly. Generalising it to a `PayoutInstruction` is a change to a reviewed interface plus its conformance kit, so it belongs in its own PR (Â§15.2).

### PayFac â€” sub-merchant trees (Â§6.1)

A merchant may now have children. `merchants.parent_merchant_id` is the tree (NULL = top of its own tree, which is what every merchant already was), and `merchants.settling_party` is the spec's one hard design constraint made a column rather than an assumption â€” `docs/SPEC-PAY-VERTICALS-2026-08-02.md` Â§2: _"model the sub-merchant relationship so the settling party is a field, not an assumption."_

**This slice moves no value.** No ledger client, no recipe, no balance, no amount. A sub-merchant is a sovereign account exactly as a merchant is; the tree decides **who may ask**, never where money sits (Doctrine Â§0.6).

| Procedure                       | Scope       | Input                                                    | Output              |
| ------------------------------- | ----------- | -------------------------------------------------------- | ------------------- |
| `submerchant.create`            | `pay:write` | `{ parentMerchantId, userId, pricing: { feeBps }, â€¦ }`   | `SubMerchant`       |
...
| `submerchantPermission.areas`   | `pay:read`  | â€“                                                        | the area vocabulary |

#### The two checks, which are not the same check

1. **Scope** â€” structural and absolute. The caller's merchant node must be an ancestor of the subject, or the subject itself. **No grant widens this.** A parent cannot read a sibling subtree, a child cannot read upward, and two payfacs are invisible to each other.
2. **Area** â€” a permission, and only for a subject that is not the caller itself. A merchant holds every area over its **own** node. Over a descendant it holds an area only if it is the **root** of that tree (the node with the platform relationship, which Â§2 of the spec makes liable for everyone beneath it), or a live grant says so.
...
**Delegation flows down, and only what you hold.** A non-root node cannot self-grant, cannot grant laterally (one child never gets authority over another), and cannot grant an area it was not given. Onboarding writes exactly two default areas to each intermediate ancestor â€” `merchant.profile` and `submerchant`, visibility only. **No value-shaped area is held by any non-root node until somebody grants it by name and says why.**
...
#### The "14 permission areas" â€” said out loud

The tracker row is titled _"PayFac mode â€” sub-merchant trees, 14 permission areas"_. **That list has never existed.** The phrase is one title string copied between `tooling/tracker/features.mjs`, `tooling/coverage.yaml`, `INTAFACED_DEFINITIVE_BUILD.md` and three board renders derived from them; `docs/PAY-LANE-HARVEST-AND-BUILD-PLAN-2026-08-08.md` Â§2 found the same thing independently and Â§6.3 puts _"enumerate them, or drop the claim from the title"_ on the owner's list.

So this ships the **mechanism**, and `PERMISSION_AREAS` in `src/submerchants.ts` is **eleven** areas â€” every one that names a surface this service actually has: `merchant.profile`, `checkout.profile`, `payment.link`, `payment`, `payment.refund`, `settlement`, `settlement.payout`, `webhook`, `kyb`, `submerchant`, `permission`. Padding the list to hit a number in a title would be inventing product law inside an implementation. `area` is stored as text, so the twelfth is a one-line change and no migration.

#### What is deliberately NOT here

- **Settling a sub-merchant out of our account.** `settlingParty` accepts `'self'` only and refuses everything else by name (`pay.submerchant_settling_party_unsupported`). That is acquiring â€” a sponsor bank and an acquiring BIN, which `docs/SPEC-PAY-VERTICALS-2026-08-02.md` Â§8 puts on the owner's list and `socket.psp-partners` tracks. The column exists so adopting a partner later is configuration rather than a rewrite; it is not a switch this service can throw.
- **Areas on the gateway surface (W5).** Money and merchant procedures in `router.ts` / `public-rest.ts` call `assertMerchantAreaAccess` with the matching area (`payment`, `payment.refund`, `settlement`, `settlement.payout`, `payment.link`, `webhook`, `kyb`, â€¦). Self still holds every area. A parent without a grant is `pay.submerchant_permission_denied`; a stranger stays `pay.merchant_forbidden`. Pins: `merchant-ownership.test.ts`.
- **Split payments and sub-merchant fee routing.** Ledger recipes, Class M, and the owner's sign-off (`docs/PAY-LANE-HARVEST-AND-BUILD-PLAN-2026-08-08.md` Â§6.4).
...
`pay.submerchant_out_of_scope` and `pay.submerchant_permission_denied` map to **FORBIDDEN**, and they are different questions on purpose: the first says the node is not yours to look at, the second says it is yours to look at and not yours to act on. `pay.submerchant_user_already_merchant` maps to **CONFLICT** â€” one merchant per sovereign account is a database rule and nothing the caller resends fixes it. `pay.submerchant_cycle` is **CONFLICT** rather than a 500 so the message naming the corrupted node survives to an operator.
...
### `payment_events` â€” the internal append-only history

This is the state history Â§6.1 requires, and it is the truth this service reasons from. Ordered by `seq` (a `bigserial`), not by `ts`: every event appended inside one transaction shares a transaction timestamp, and a history that cannot order a capture against a refund is not a history.

| Event                 | Appended when                                                                                                               |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `created`             | A payment row exists. Nothing has moved.                                                                                    |
| `rail.authorize`      | A rail answered an authorization request (ok or not).                                                                       |
| `rail.pending`        | The rail took the request but has not completed it â€” on `crypto-native`, the transfer has not landed or is not deep enough. |
...
| `webhook.<type>`      | A verified rail delivery. Carries `rail_event_id`, which is **unique** â€” this is the dedupe.                                |

### NATS

**svc-pay publishes no NATS subjects yet.** Not an oversight: the bus is a contract, and Â§15.2 / AGENT_PROTOCOL Â§2 require a `packages/events` catalog PR â€” reviewed on its own â€” before a producer emits anything. `intafaced.pay.payment.captured` and `intafaced.pay.settlement.completed` are the two consumers will want; they land in the catalog first.
...
Every value movement is a recipe. This service holds no balance of any kind (Doctrine Â§0.6).

| Recipe                 | Reason code               | Accounts                                                               |
| ---------------------- | ------------------------- | ---------------------------------------------------------------------- |
| `paymentCapture`       | `payment.captured`        | rail boundary â†’ `pay:clearing:<merchantId>`                            |
| `merchantSettlement`   | `pay.settled`             | `pay:clearing:<merchantId>` â†’ merchant available + `houseFees('pay')`  |
| `paymentRefund`        | `payment.refunded`        | clearing (pre-settlement) or merchant available (post) â†’ rail boundary |
| `paymentRefundReverse` | `payment.refund.reversed` | rail boundary â†’ back where the refund came from                        |
| `withdrawHold`         | `withdraw.held`           | available â†’ purpose-keyed hold (payout / withdrawal in flight)         |
| `withdrawSettle`       | `withdraw.settled`        | hold â†’ rail boundary (completed)                                       |
| `withdrawReverse`      | `withdraw.reversed`       | hold â†’ available (refused)                                             |
| `deposit`              | `deposit.credited`        | rail boundary â†’ user available                                         |

The three `withdraw*` recipes serve **both** outbound paths â€” a merchant payout keyed on `<settlementId>:<attempt>`, and a user withdrawal keyed on `<withdrawalId>:<attempt>`. Same shape, same reasoning, one set of recipes.
...
`withdrawHold` â†’ `withdrawSettle` / `withdrawReverse` requests. Rail adapters
...
**`merchantClearing(merchantId, assetId)`** is a new account constructor: a `module`-owned account per merchant, per asset. It answers "a payment was captured but not settled â€” whose funds are those?" as a balance rather than an investigation. `sum(merchantClearing(m))` is exactly what svc-pay owes merchant `m` right now, readable from the ledger without touching a single svc-pay table â€” which is what makes reconciliation between the two meaningful.

### Idempotency keys â€” all business keys, never random
...
> **Why the asset is in the settlement key.** Â§6.1's settlement is keyed on merchant and window. A merchant taking USDT and BTC on the same day has two settlements, and without the asset in the key the second would find the first's transaction, return it, and strand a whole currency's takings in clearing. The `settlements` table carries an `asset_id` column for the same reason: `gross`/`fees`/`net` are meaningless without one.

### Order of operations, and whose funds are stranded

The direction of the money decides the order:

- **Inbound (capture):** the rail moves first, the ledger books second. We only book value we know has arrived. A crash in between leaves money captured at the rail and not yet in the book â€” the classic. Nothing is lost: both halves are keyed on the payment id, so re-running finishes the job.
...
A refund whose id is already in flight is **refused**, not re-sent. Live crypto receives a durable `refundId` (M226-02) so a process restart reuses the same outbound broadcast key; card-sandbox ignores it. The **service** still refuses re-entry while `refund.posted` has no terminal `refunded` / `refund.reversed` â€” "send it again and hope" is how one refund becomes two on a rail that cannot dedupe.

### User money: whose funds are stranded, per branch

**Deposit** (inbound â€” the rail already moved, in the real world):
...
| Claimed, not yet booked      | **The user's** â€” at the rail, not in the book. Row is `pending`.  | Re-run the same `(rail, railRef)`; `deposits_status_idx` lists exactly these. |
...
**Withdrawal** (outbound â€” the ledger moves first):
...
| **Held, rail never asked**   | **The user's, immobilised** â€” out of `available`, in `withdraw:<id>:<attempt>`. The only branch where value is stuck. | Re-run on the same `clientRef`: the hold key is identical so it moves nothing, and the rail is then asked. `withdrawals_status_idx` lists everything in `held`. |
...
`src/rails/rail-adapter.ts` is the interface, exactly as Â§6.1 specifies â€” `id`, `capabilities`, `authorize`, `capture`, `refund`, `payout`, `verifyWebhook` â€” plus `health()`, taken from the shape `packages/venue-adapter` already uses for liquidity venues.
...
- **authorize** means "a transfer to this payment's acceptance address has reached `minConfirmations`". Below the threshold the answer is `pending`, not `authorized` â€” a shallow transaction can still be reorganised away, and a merchant who ships against one has shipped against nothing.
...
Constant-time, via `crypto.timingSafeEqual`, over the **raw body** signed together with a timestamp. Length is checked separately first, because `timingSafeEqual` throws on a length mismatch and an unhandled exception on a public endpoint is a denial-of-service surface. Non-hex signatures are rejected outright: `Buffer.from('zz','hex')` is empty, and two empty buffers compare equal â€” a signature of nothing verifying nothing. Stale timestamps are replays. Nothing throws, whatever arrives.
...
### Adding a rail â€” the conformance kit (Â§6.3)
...
What the kit asserts is exactly what the core is entitled to assume. That is what makes Â§6.1's "drop in later as adapters with zero core changes" a testable claim instead of a hope.

---

## Kill-switch

`module.pay` in the admin console â€” the module kill-switch beats every other flag (`packages/config/src/flags.ts`). With it off, no procedure in this service serves. `pay.payfac` and `pay.laneA` gate features not built here.
...
Tests run against real Postgres (`postgres://svc_pay:svc_pay@localhost:5433/intafaced`) with `MemoryLedger` as the ledger â€” legitimate because the ledger conformance suite proves the reference implementation and svc-ledger's Postgres engine behave identically (Â§4.4). Postgres is real because the payment row / event log / ledger interaction is exactly where a bug would hide, because the append-only guarantee is a database trigger an in-memory fake would quietly not have, and because the deposit and withdrawal idempotency **is** a unique index.

The two Postgres suites bring the schema up under a shared advisory lock and own disjoint tables â€” vitest runs test files in parallel, and both re-asserting the same CHECK constraints at once deadlocks.
