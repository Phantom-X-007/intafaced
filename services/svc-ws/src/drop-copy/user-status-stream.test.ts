...
import type { AccountStatusSnapshot, LiveCredentialPort, OwnershipSnapshot } from '../private/live-credential.js';
...
describe('drop-copy stream drops when the user is frozen', () => {
  let server: Server;
  ...
  async function boot(liveCredential: LiveCredentialPort, heartbeatMs = 30_000): Promise<{ host: string; port: number }> {
    const hub = new DropCopyHub({ highWaterBytes: 1_000_000, maxLagTicks: 5, maxConnections: 10 });
  ...
  it('active user upgrades on session and API-key seats', async () => {
    ...
    expect(await upgradeStatus(host, port, `${DROP_COPY_STREAM_PATH}?access_token=${session}`)).toBe(101);
    expect(await upgradeStatus(host, port, `${DROP_COPY_STREAM_PATH}?access_token=${key}`)).toBe(101);
  });

  it('frozen, closed, and missing account refuse with 401', async () => {
    const frozen = await boot(accountPort('frozen'));
    const token = await access();
    expect(await upgradeStatus(frozen.host, frozen.port, `${DROP_COPY_STREAM_PATH}?access_token=${token}`)).toBe(401);
    await gateway.close('reset');
    await new Promise<void>((resolve) => server.close(() => resolve()));

    const closed = await boot(accountPort('closed'));
    expect(await upgradeStatus(closed.host, closed.port, `${DROP_COPY_STREAM_PATH}?access_token=${token}`)).toBe(401);
    await gateway.close('reset');
    await new Promise<void>((resolve) => server.close(() => resolve()));

    const missing = await boot(accountPort(null));
    expect(await upgradeStatus(missing.host, missing.port, `${DROP_COPY_STREAM_PATH}?access_token=${token}`)).toBe(401);
  });
  ...
  it('frozen user never receives a ready frame', async () => {
    ...
    const token = await access();
    const frames: string[] = [];
    const ws = new WebSocket(`ws://127.0.0.1:${listen}${DROP_COPY_STREAM_PATH}?access_token=${token}`);
