...
function stubDeps(
  ...
  sessionLog: (sessionId: string, kind?: AuditedAction['kind']) => Promise<AuditedAction[]>,
): AgentsRouterDeps {
  return {
...
describe('session.log kind filter', () => {
  it('omits kind and still returns mixed kinds in sequence for an owned session', async () => {
    const calls: Array<{ sessionId: string; kind?: AuditedAction['kind'] }> = [];
    const mixed = [
    ...
    expect(calls).toEqual([{ sessionId: SESSION, kind: undefined }]);
  ...
  it('passes an exact kind after ownedSession', async () => {
    ...
    expect(calls).toEqual([{ sessionId: SESSION, kind: 'tool_call' }]);
