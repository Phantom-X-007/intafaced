...
**`dev` is frictionless, `staging`/`prod` are explicit.** With nothing configured, `dev`/`test` fall back to `http://localhost:3000` / `http://127.0.0.1:3000` / `:3100` (our own front-ends, both loopback spellings). `staging` and `prod` get no default.
