...
import { expirePaperTrailingStopParent } from './oms-paper-trailing-stop-expire.js';
...
describe('expirePaperTrailingStopParent', () => {
  it('refuses missing / whitespace parentClientOrderId', () => {
    expect(
      expirePaperTrailingStopParent({
        status: 'paper',
        expireAt: EXPIRE_AT,
        remaining: '10',
        paper: PAPER_ON,
  ...
  it("happy: status paper + expireAt + remaining '10' + paper on", () => {
    const expired = expirePaperTrailingStopParent({
      parentClientOrderId: 'p-trail',
      status: 'paper',
      expireAt: EXPIRE_AT,
      remaining: '10',
