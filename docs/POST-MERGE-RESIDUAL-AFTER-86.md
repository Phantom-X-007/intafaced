...
**Claim tags:** `[VERIFIED 2026-07-29]` against #86 branch evidence + STATUS/LICENCE docs · residual **#2–#4 closed** by vendor shell hygiene PR (this branch).

---

## One-line floor

**Three custody doors closed on main with #86; wallet mass-credit + CORS `*` closed in residual hygiene.** Real user money still blocked: dual-book discipline, wallet secrets/perimeter, pay retry contract, licence Priority-1, rails/chain/kill-path.
...
| #   | Item                                                       | Owner                                           | When                                     |
| --- | ---------------------------------------------------------- | ----------------------------------------------- | ---------------------------------------- |
| 1   | Vendor UI ≠ money books (TS ledger remains books)          | Nitro policy · all agents                       | **Now** (PEACE) · hard before real money |
...
| 8   | Pay withdraw retry: test vs service contract               | Money agent (avoid colliding open pay PRs)      | Soon · before real money                 |
| 9   | CI money tests can skip without PG env                     | Agent tooling                                   | Soon (harness honesty)                   |
...
- Taking open feature PRs (#87–#94 class) from the product-shipping chat
