...
**In progress:** OTC RFQ desk, staked-tier gate (cursor-swarm-otc) · Copy trading, audited leaders, profit share (Nitro) · TWAP / VWAP / POV execution (Nitro) · Pro terminal — depth, charts, hotkeys, sub-accounts (Nitro) · WebSocket fan-out: depth, trades, orders, positions (Nitro) · Branded gateway, hosted checkout, payment links (Nitro) · Moderated dispute resolution (nitro-agent) · Payment instruments — where the buyer actually pays (nitro-agent) · Fiat on/off ramp reusing svc-pay adapters (cursor-swarm-bank) · Residencies, IFC pay, revenue share (Nitro) · Support desk, tickets, KB (cursor-swarm-ops-support) · Multi-tier affiliate / IB trees, payout automation (cursor-swarm-affiliates) · Warehouse — read replica + cube layer (cursor-swarm-analytics)
...
- `pay.settlement` — Dual settlement — bank or crypto
- …and 14 more
