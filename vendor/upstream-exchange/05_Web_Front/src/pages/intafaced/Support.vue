...
.support-actions { margin: 0; border-top: 1px solid #282828; }
...
.support-actions > summary:focus-visible { outline: 2px solid var(--ix-orange); outline-offset: 2px; }
</style>
...
export default {
  ...
  methods: {
    ...
    postComment() {
      var self = this;
      var ticketId = this.openedId;
      var body = this.commentBody;
      if (!ticketId || !body) return;
      this.act('comment', mutate('support', 'comment', {ticketId, body}, this.ixToken)).then(function(res) {
        if (res && res.ok) {
          self.commentBody = '';
          self.loadComments();
          self.loadTickets();
