...
| **CODEOWNERS** | File that auto-requests review from named people when paths change. On Free it *asks*; it does not *block* merge. |
...
### What Denon *did* enable (server-side)
...
### Critical take for *this* codebase
...
| **`apps/web` (Next.js App Router)** | **Yes.** Folder under `src/app/` **is** the URL (`app/trade/page.tsx` → `/trade`). | Name folders for the URL path. That *is* Denon’s rule, already how Next works. No extra convention needed. |
