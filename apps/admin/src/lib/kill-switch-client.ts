 ...
 * sent anywhere." §14.6 asks for a kill-switch an operator can reach, and
 * nothing here reached anything.
 *
 ...
 * other process. `svc-trade`, `svc-ws` and the rest read their switch from an
 * environment variable once, at boot. The honest summary of the state before
 * this change: every kill-switch in the platform required a redeploy, and the
 * one surface that was supposed to reach them reached nothing.
 *
 * This calls `svc-edge`'s `/admin/kill-switches`, which enforces the switch on
 * the request path in front of every service (see `services/svc-edge/src/
 * kill-switch.ts` for why the edge and not each service).
 *
 * ── The credential ──────────────────────────────────────────────────────────
 *
 * `ADMIN_OPERATOR_TOKEN` — an access token carrying `admin:write` with `mfa`,
 * issued to the operator, held server-side and never sent to the browser.
 *
 * That is a real limitation and it is stated rather than hidden: this console
 * has no login of its own (its README has said so from the start), so the token
 * identifies the CONSOLE rather than the human at it. The edge's audit line
 * therefore names one operator identity per deployment. When operator SSO lands
 * — the same §13 socket the README already names — this function takes the
 * signed-in operator's own token and the audit line names the person.
 *
 ...
 * `verifyAccessToken` + `requireScope` + `requireMfa` path as every other
...
export type ControlPlaneStatus =
  ...
  | 'reachable'
  /** No `ADMIN_OPERATOR_TOKEN` / `EDGE_URL` — the console must not pretend. */
  | 'unconfigured'
...
const EMPTY: KillSwitchSnapshot = { disabledModules: [], reasons: {} };

function config(): { edgeUrl: string; token: string } | null {
  const edgeUrl = process.env.EDGE_URL;
  const token = process.env.ADMIN_OPERATOR_TOKEN;
...
export async function readKillSwitches(): Promise<ControlPlaneState> {
  ...
  try {
    ...
    const body = (await res.json()) as KillSwitchSnapshot;
    return { status: 'reachable', snapshot: { disabledModules: body.disabledModules ?? [], reasons: body.reasons ?? {} }, detail: null };
  } catch (err) {
...
export async function setKillSwitch(input: ToggleInput): Promise<ToggleResult> {
  ...
  try {
    ...
    return {
      ok: true,
      status: res.status,
      snapshot: { disabledModules: body.disabledModules ?? [], reasons: body.reasons ?? {} },
      detail: null,
    };
  } catch (err) {
