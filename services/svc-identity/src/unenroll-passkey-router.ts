...
import { unenrollPasskey, UnenrollPasskeyError } from './auth/unenroll-passkey.js';

/**
 * Top-level unenroll (not nested under webauthn) so mergeRouters cannot replace
 * enroll/verify. identity:write. Drops the stored cred. No invented challenge.
 */
export function createUnenrollPasskeyRouter(sql: Sql) {
  return router({
    unenrollPasskey: scopedProcedure('identity:write')
      ...
      .output(z.object({ credentialId: z.string(), remaining: z.number().int() }))
      .mutation(async ({ ctx, input }) => {
        try {
          return await unenrollPasskey(sql, ctx.principal.userId, input.credentialId);
        } catch (err) {
