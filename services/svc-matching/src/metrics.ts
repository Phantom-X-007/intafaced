 ...
 *
 * ── Why matching, when the SLO is measured at the edge ──────────────────────
 *
 ...
 * the engine: submit / depth / cancel latency on THIS hop, so an operator can
 * tell "the engine is the slow part" from "the queue in front of it is".
 *
 * Matching is not a proxy. Every request this process answers is local work, so
 ...
 *
 * Always `_local`. Matching has no `UPSTREAMS` table and no `/api/` prefix to
    ...
    registry.observe(
      {
        ...
        // Matching is not a proxy and does not run principal exchange. Service
