/**
 * Residual-release leftover already on an expired paper scale-in parent.
 * Hands the retained leftover through ledger-client. This door never invents an
 * amount from childSize, parent amount, or the clock, never invents a live venue,
 * and does not touch matching.
 */
import { formatAmount, parseAmount } from '@intafaced/ledger-client';
import type { PaperGate } from './oms-paper.js';

export type OmsPaperScaleInReleaseResidualRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'not_expired'
  | 'already_released'
...
export type OmsPaperScaleInReleaseResidualRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperScaleInReleaseResidualRefuseReason;
  readonly detail: string;
};

export type OmsPaperScaleInReleaseResidualOk = {
  readonly ok: true;
...
export type OmsPaperScaleInReleaseResidualResult =
  | OmsPaperScaleInReleaseResidualOk
  | OmsPaperScaleInReleaseResidualRefusal;

function refuse(
  reason: OmsPaperScaleInReleaseResidualRefuseReason,
  detail: string,
): OmsPaperScaleInReleaseResidualRefusal {
...
): { ok: true; text: string } | OmsPaperScaleInReleaseResidualRefusal {
  if (raw === null || raw === undefined) {
    return refuse(
      'missing_residual',
      'residual.remaining is missing — refusing to invent leftover from childSize or the clock',
    );
  }
  const text = raw.trim();
  if (text.length === 0) {
...
/**
 * Residual-release leftover already retained on an expired paper scale-in parent through ledger-client.
 */
export function releaseExpiredPaperScaleInResidual(input: {
  ...
  paper?: PaperGate;
}): OmsPaperScaleInReleaseResidualResult {
