...
D26-P1-A2 seals three more honesty doors on that spine: tool calls are **stoppable** (`AbortSignal` mid-loop returns `stopped` after settle/close, billed amount stays honest, no silent leftover feeCharge); an account fixture that smuggles money keys (`balance`, `amount`, …) is refused as `balance_field_forbidden` rather than strip-and-lie; and when account-state was asked and missing/invent, the whole run refuses `account_state_missing` so a KB cite cannot dress up as a grounded account check.
