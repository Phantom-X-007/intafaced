...
export class AuthService {
  ...
  async login(input: { identifier: string; password: string; totpCode?: string; device?: string; ip?: string }): Promise<SessionTokens> {
    const rows = await this.sql<Array<{ id: string; password_hash: string; status: string; totp_secret: string | null }>>`SELECT id, password_hash, status, totp_secret FROM users WHERE handle = ${input.identifier} OR email = ${input.identifier}`;
    const user = rows[0];
  ...
  private async issueSession(userId: string, options: { device?: string | undefined; ip?: string | undefined; mfa: boolean }): Promise<SessionTokens> {
    const refreshToken = generateToken(48);
    const expiresAt = new Date(Date.now() + this.tokens.refreshTtlSeconds * 1000);
    const inserted = await this.sql<Array<{ id: string }>>`INSERT INTO sessions (user_id, refresh_hash, device, ip, mfa, expires_at) VALUES (${userId}, ${hashToken(refreshToken)}, ${options.device ?? null}, ${options.ip ?? null}, ${options.mfa}, ${expiresAt}) RETURNING id`;
    const sessionId = inserted[0]!.id;
    void syncNavigatorSessionOpen(this.sql, sessionId, userId);
    const tier = await this.kycTier(userId);
    const { token: accessToken, expiresAt: accessExpiresAt } = await issueAccessToken({ userId, sessionId, scopes: this.defaultScopes(), tier, mfa: options.mfa }, this.tokens);
  ...
  async refresh(refreshToken: string, options: { device?: string; ip?: string } = {}): Promise<SessionTokens> {
    ...
    const result = await transaction<RefreshOutcome>(this.sql, async (tx) => {
      const rows = await tx<Array<{ id: string; user_id: string; revoked: boolean; mfa: boolean; expires_at: Date; user_status: string }>>`SELECT s.id, s.user_id, s.revoked, s.mfa, s.expires_at, u.status AS user_status FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.refresh_hash = ${hash} FOR UPDATE OF s, u`;
      ...
      const expiresAt = new Date(Date.now() + this.tokens.refreshTtlSeconds * 1000);
      const nextSession = await tx<Array<{ id: string }>>`INSERT INTO sessions (user_id, refresh_hash, device, ip, mfa, expires_at) VALUES (${session.user_id}, ${hashToken(nextToken)}, ${options.device ?? null}, ${options.ip ?? null}, ${session.mfa}, ${expiresAt}) RETURNING id`;
      const sessionId = nextSession[0]!.id;
      const tier = await this.kycTier(session.user_id, tx);
      const { token: accessToken, expiresAt: accessExpiresAt } = await issueAccessToken({ userId: session.user_id, sessionId, scopes: this.defaultScopes(), tier, mfa: session.mfa }, this.tokens);
      return { kind: 'rotated', previousSessionId: session.id, tokens: { accessToken, refreshToken: nextToken, expiresAt: accessExpiresAt, sessionId, userId: session.user_id, mfaRequired: false } };
    });
  ...
  async getSessionOwnership(sessionId: string): Promise<{ id: string; userId: string; revoked: boolean } | null> {
    const rows = await this.sql<Array<{ id: string; user_id: string; revoked: boolean }>>`SELECT id, user_id, revoked FROM sessions WHERE id = ${sessionId} LIMIT 1`;
    const row = rows[0];
    if (!row) return null;
  ...
  async assertSessionLive(sessionId: string): Promise<{ id: string; userId: string }> {
    ...
    const rows = await this.sql<Array<{ id: string; user_id: string; revoked: boolean; expires_at: Date }>>`SELECT id, user_id, revoked, expires_at FROM sessions WHERE id = ${id} LIMIT 1`;
    const row = rows[0];
    if (!row) {
      throw new AuthError('Session not found', 'auth.session_denied');
    }
  ...
  private async tryRedeemRecoveryCode(userId: string, code: string): Promise<boolean> {
    ...
    return transaction(this.sql, async (tx) => {
      const rows = await tx<Array<{ recovery_code_hashes: unknown }>>`SELECT recovery_code_hashes FROM users WHERE id = ${userId} FOR UPDATE`;
  ...
  private async consumeTotpCode(userId: string, secret: string, code: string, at?: Date): Promise<void> {
    ...
    await transaction(this.sql, async (tx) => {
      const rows = await tx<Array<{ totp_last_step: string | number | bigint | null }>>`SELECT totp_last_step FROM users WHERE id = ${userId} FOR UPDATE`;
  ...
  async startTotpEnrolment(userId: string): Promise<{ secret: string; uri: string; recoveryCodes: string[] }> {
    if (!this.totpSecretKey) {
      throw new AuthError('IDENTITY_TOTP_SECRET_KEY is not set to a 32-byte key (base64 or hex) — cannot enrol TOTP', 'auth.totp_key_missing');
    }
    const rows = await this.sql<Array<{ email: string; totp_secret: string | null }>>`SELECT email, totp_secret FROM users WHERE id = ${userId}`;
    const user = rows[0];
    ...
    await this.pendingTotp.put(userId, hashToken(secret), recoveryCodes.map((c) => hashToken(c)));
  ...
  async confirmTotpEnrolment(userId: string, secret: string, code: string): Promise<void> {
    ...
    await this.sql`UPDATE users SET totp_secret = ${sealed}, totp_enrolled_at = now(), totp_last_step = ${matched.toString()}, recovery_code_hashes = ${this.sql.json(pending.recoveryHashes as never)}, updated_at = now() WHERE id = ${userId} AND totp_secret IS NULL`;
    await this.rank.awardXp({ userId, sourceModule: 'identity', action: 'identity.totp.enrolled', xpDelta: 100, idempotencyKey: `identity.totp.enrolled:${userId}` });
  }

  async startWebauthnRegistration(userId: string): Promise<RegistrationOptionsJSON> {
    const rows = await this.sql<Array<{ email: string; handle: string; webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT email, handle, webauthn_creds FROM users WHERE id = ${userId}`;
    const user = rows[0];
    if (!user) throw new AuthError('User not found', 'auth.not_found');
  ...
  async confirmWebauthnRegistration(userId: string, response: RegistrationResponseJSON): Promise<{ credentialId: string }> {
    const rows = await this.sql<Array<{ webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT webauthn_creds FROM users WHERE id = ${userId}`;
    const user = rows[0];
    if (!user) throw new AuthError('User not found', 'auth.not_found');
    ...
    if (existing.length === 0) {
      await this.rank.awardXp({ userId, sourceModule: 'identity', action: 'identity.webauthn.enrolled', xpDelta: 100, idempotencyKey: `identity.webauthn.enrolled:${userId}` });
    }
  ...
  async startWebauthnAuthentication(identifier: string): Promise<AuthenticationOptionsJSON> {
    const rows = await this.sql<Array<{ id: string; status: string; webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT id, status, webauthn_creds FROM users WHERE handle = ${identifier} OR email = ${identifier}`;
    const user = rows[0];
    const challenge = generateChallenge();
  ...
  async confirmWebauthnAuthentication(identifier: string, response: AuthenticationResponseJSON, options: { device?: string; ip?: string } = {}): Promise<SessionTokens> {
    const rows = await this.sql<Array<{ id: string; status: string; webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT id, status, webauthn_creds FROM users WHERE handle = ${identifier} OR email = ${identifier}`;
    const user = rows[0];
  ...
  async listWebauthnCredentials(userId: string): Promise<Array<{ credentialId: string; createdAt: string; transports?: string[] }>> {
    const rows = await this.sql<Array<{ webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT webauthn_creds FROM users WHERE id = ${userId}`;
    if (!rows[0]) throw new AuthError('User not found', 'auth.not_found');
    return asCredentialList(rows[0].webauthn_creds).map((c) => ({ credentialId: c.credentialId, createdAt: c.createdAt, ...(c.transports ? { transports: c.transports } : {}) }));
  }
  ...
  async startWebauthnStepUp(userId: string) {
    const rows = await this.sql<Array<{ id: string; status: string; webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT id, status, webauthn_creds FROM users WHERE id = ${userId}`;
    const user = rows[0];
    if (!user) throw new AuthError('User not found', 'auth.not_found');
    if (user.status !== 'active') throw new AuthError(`Account is ${user.status}`, 'auth.account_frozen');
    const creds = asCredentialList(user.webauthn_creds);
  ...
  async createApiKey(input: { userId: string; name: string; scopes: string[]; grantorScopes: readonly string[]; domainWhitelist?: string[]; expiresAt?: Date; mode?: 'live' | 'sandbox' }): Promise<{ id: string; key: string; prefix: string; mode: 'live' | 'sandbox' }> {
    ...
    const rows = await this.sql<Array<{ id: string }>>`INSERT INTO api_keys (user_id, name, key_hash, key_prefix, scopes, domain_whitelist, expires_at, mode) VALUES (${input.userId}, ${input.name}, ${hash}, ${prefix}, ${input.scopes}, ${input.domainWhitelist ?? []}, ${input.expiresAt ?? null}, ${mode}) RETURNING id`;
    return { id: rows[0]!.id, key, prefix, mode };
  }

  async verifyApiKey(key: string): Promise<{ userId: string; scopes: string[]; keyId: string; mode: 'live' | 'sandbox'; domainWhitelist: string[] } | null> {
    const rows = await this.sql<Array<{ id: string; user_id: string; scopes: string[]; expires_at: Date | null; mode: string | null; domain_whitelist: string[] | null }>>`SELECT id, user_id, scopes, expires_at, mode, domain_whitelist FROM api_keys WHERE key_hash = ${hashToken(key)} AND revoked = false`;
    const row = rows[0];
    if (!row) return null;
  ...
  async exchangeApiKey(key: string, requestOrigin?: string | null): Promise<{ accessToken: string; expiresAt: Date; userId: string; keyId: string; scopes: string[]; mode: 'live' | 'sandbox' }> {
  ...
  async getApiKeyOwnership(keyId: string): Promise<{ id: string; userId: string; revoked: boolean } | null> {
    const rows = await this.sql<Array<{ id: string; user_id: string; revoked: boolean }>>`SELECT id, user_id, revoked FROM api_keys WHERE id = ${keyId} LIMIT 1`;
    const row = rows[0];
    if (!row) return null;
    return { id: row.id, userId: row.user_id, revoked: row.revoked };
  }
  ...
  async listApiKeys(userId: string) {
    return this.sql<Array<{ id: string; name: string; key_prefix: string; scopes: string[]; last_used_at: Date | null; revoked: boolean; mode: string }>>`SELECT id, name, key_prefix, scopes, last_used_at, revoked, mode FROM api_keys WHERE user_id = ${userId} ORDER BY created_at DESC`;
  }

  async kycTier(userId: string, sql: Sql = this.sql): Promise<'none' | 'basic' | 'full' | 'institutional'> {
    const rows = await sql<Array<{ tier: 'basic' | 'full' | 'institutional' }>>`SELECT tier FROM kyc_records WHERE user_id = ${userId} AND status = 'approved' AND (expires_at IS NULL OR expires_at > now())`;
  ...
  async accountState(userId: string): Promise<AccountState | null> {
    const rows = await this.sql<Array<{ id: string; status: 'active' | 'frozen' | 'closed' }>>`SELECT id, status FROM users WHERE id = ${userId} LIMIT 1`;
    const user = rows[0];
    if (!user) return null;
  ...
  async submitKyc(input: { userId: string; tier: SubmittableKycTier; jurisdiction: string; providerRef?: string }): Promise<KycRecordView> {
    return transaction(this.sql, async (tx) => {
      ...
      const existing = await tx<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE user_id = ${input.userId} AND ((status = 'approved' AND (expires_at IS NULL OR expires_at > now())) OR (status = 'pending' AND tier = ${input.tier})) ORDER BY created_at DESC`;
      ...
      const inserted = await tx<KycRow[]>`INSERT INTO kyc_records (user_id, tier, jurisdiction, provider_ref, status) VALUES (${input.userId}, ${input.tier}, ${input.jurisdiction}, ${input.providerRef ?? null}, 'pending') RETURNING id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at`;
      return toKycRecord(inserted[0]!);
    });
  }

  async listKycRecords(userId: string): Promise<KycRecordView[]> {
    const rows = await this.sql<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE user_id = ${userId} ORDER BY created_at DESC`;
    return rows.map(toKycRecord);
  }

  async listPendingKyc(limit = 50): Promise<KycRecordView[]> {
    const rows = await this.sql<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE status = 'pending' ORDER BY created_at ASC LIMIT ${limit}`;
    return rows.map(toKycRecord);
  }

  async getKycRecord(recordId: string): Promise<KycRecordView | null> {
    const rows = await this.sql<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE id = ${recordId}`;
    return rows[0] ? toKycRecord(rows[0]) : null;
  }

  async approveKycRecord(input: { recordId: string; reviewerId: string; expiresAt?: Date | null; service?: string | null; kid?: string | null }): Promise<KycRecordView> {
    assertOperatorKycReview({ service: input.service, kid: input.kid });
    const outcome = await transaction(this.sql, async (tx) => {
      const rows = await tx<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE id = ${input.recordId} FOR UPDATE`;
      const row = rows[0];
      ...
      const updated = await tx<KycRow[]>`UPDATE kyc_records SET status = 'approved', reviewed_at = now(), reviewed_by = ${input.reviewerId}, expires_at = ${input.expiresAt ?? null} WHERE id = ${row.id} RETURNING id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at`;
  ...
  async rejectKycRecord(input: { recordId: string; reviewerId: string; service?: string | null; kid?: string | null }): Promise<KycRecordView> {
    assertOperatorKycReview({ service: input.service, kid: input.kid });
    return transaction(this.sql, async (tx) => {
      const rows = await tx<KycRow[]>`SELECT id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at FROM kyc_records WHERE id = ${input.recordId} FOR UPDATE`;
      const row = rows[0];
      ...
      const updated = await tx<KycRow[]>`UPDATE kyc_records SET status = 'rejected', reviewed_at = now(), reviewed_by = ${input.reviewerId} WHERE id = ${row.id} RETURNING id, user_id, tier, jurisdiction, provider_ref, status, reviewed_by, reviewed_at, expires_at, created_at`;
  ...
  async approveKyc(input: { userId: string; tier: SubmittableKycTier; jurisdiction: string; providerRef?: string }): Promise<void> {
    await this.sql`INSERT INTO kyc_records (user_id, tier, jurisdiction, provider_ref, status, reviewed_at) VALUES (${input.userId}, ${input.tier}, ${input.jurisdiction}, ${input.providerRef ?? null}, 'approved', now())`;
  ...
  private async announceKycApproval(record: { userId: string; tier: KycTier; jurisdiction: string }): Promise<void> {
    ...
    await this.bus.publish('kycApproved', { userId: record.userId, tier: record.tier, jurisdiction: record.jurisdiction }, { idempotencyKey: `kyc.approved:${record.userId}:${record.tier}` });
    await this.rank.awardXp({ userId: record.userId, sourceModule: 'identity', action: 'identity.kyc.approved', xpDelta: 200, idempotencyKey: `identity.kyc.approved:${record.userId}:${record.tier}` });
  }

  async stepUp(input: { userId: string; sessionId: string; totpCode?: string; webauthn?: AuthenticationResponseJSON }): Promise<{ accessToken: string; expiresAt: Date; scopes: Scope[] }> {
    ...
    const users = await this.sql<Array<{ totp_secret: string | null; status: string; webauthn_creds: StoredWebAuthnCredential[] }>>`SELECT totp_secret, status, webauthn_creds FROM users WHERE id = ${input.userId}`;
    const user = users[0];
    if (!user) throw new AuthError('User not found', 'auth.not_found');
    if (user.status !== 'active') throw new AuthError(`Account is ${user.status}`, 'auth.account_frozen');
    const sessions = await this.sql<Array<{ id: string }>>`SELECT id FROM sessions WHERE id = ${input.sessionId} AND user_id = ${input.userId} AND revoked = false AND expires_at > now()`;
    ...
    const { token, expiresAt } = await issueAccessToken({ userId: input.userId, sessionId: input.sessionId, scopes, tier, mfa: true }, { ...this.tokens, accessTtlSeconds: Math.min(this.tokens.accessTtlSeconds, STEP_UP_TTL_SECONDS) });
  ...
  async freezeIdentity(userId: string): Promise<{ userId: string; status: 'frozen'; subAccountsRevoked: number; apiKeysRevoked: number }> {
    const result = await transaction(this.sql, async (tx) => {
      ...
      const revokedRows = await tx<Array<{ id: string }>>`UPDATE sub_accounts SET revoked = true WHERE parent_user_id = ${userId} AND revoked = false RETURNING id`;
      const revokedKeys = await tx<Array<{ id: string }>>`UPDATE api_keys SET revoked = true WHERE user_id = ${userId} AND revoked = false RETURNING id`;
  ...
  async createSubAccount(userId: string, label: string, purpose?: string): Promise<{ id: string }> {
    ...
    return transaction(this.sql, async (tx) => {
      ...
      const live = await tx<Array<{ n: string }>>`SELECT count(*)::text AS n FROM sub_accounts WHERE parent_user_id = ${userId} AND revoked = false`;
      const count = Number(live[0]?.n ?? '0');
      if (count >= this.maxSubAccounts) {
        throw new AuthError(`Sub-account limit reached (${this.maxSubAccounts}). Retire a live partition or raise IDENTITY_MAX_SUB_ACCOUNTS.`, 'auth.sub_account_limit');
      }
      const rows = await tx<Array<{ id: string }>>`INSERT INTO sub_accounts (parent_user_id, label, purpose) VALUES (${userId}, ${label}, ${purpose ?? null}) RETURNING id`;
      return rows[0]!;
    });
  }

  async listSubAccounts(userId: string): Promise<Array<{ id: string; label: string; purpose: string | null; revoked: boolean; createdAt: Date }>> {
    const rows = await this.sql<Array<{ id: string; label: string; purpose: string | null; revoked: boolean; created_at: Date }>>`SELECT id, label, purpose, revoked, created_at FROM sub_accounts WHERE parent_user_id = ${userId} ORDER BY created_at DESC`;
  ...
  async revokeSubAccount(userId: string, subAccountId: string): Promise<boolean> {
    const result = await this.sql`UPDATE sub_accounts SET revoked = true WHERE id = ${subAccountId} AND parent_user_id = ${userId} AND revoked = false`;
    return result.count > 0;
  }

  async getSubAccountOwnership(subAccountId: string): Promise<{ id: string; parentUserId: string; revoked: boolean } | null> {
    const rows = await this.sql<Array<{ id: string; parent_user_id: string; revoked: boolean }>>`SELECT id, parent_user_id, revoked FROM sub_accounts WHERE id = ${subAccountId} LIMIT 1`;
    const row = rows[0];
    if (!row) return null;
  ...
  async assertSubAccountOwned(userId: string, subAccountId: string | null | undefined): Promise<{ id: string; parentUserId: string }> {
    ...
    if (!id) {
      throw new AuthError('Sub-account id is required — a missing id is a refusal, never a default to primary', 'auth.sub_account_required');
    }
  ...
  async assertSubAccountTransferDoor(userId: string, fromSubAccountId: string | null | undefined, toSubAccountId: string | null | undefined): Promise<{ fromId: string; toId: string }> {
    ...
    if (!fromId || !toId) {
      throw new AuthError('Both from and to sub-account ids are required — a missing id is a refusal, never a default to primary', 'auth.sub_account_required');
    }
    if (fromId === toId) {
...
function asCredentialList(raw: unknown): StoredWebAuthnCredential[] {
  ...
  return raw.filter((c): c is StoredWebAuthnCredential => !!c && typeof c === 'object' && typeof (c as StoredWebAuthnCredential).credentialId === 'string' && typeof (c as StoredWebAuthnCredential).publicKey === 'string' && typeof (c as StoredWebAuthnCredential).counter === 'number');
}
