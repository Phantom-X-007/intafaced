...
function keyPort(products: readonly string[]): LiveCredentialPort {
  const snap: OwnershipSnapshot = { id: KEY, userId: USER, revoked: false, productScopes: products };
  return {
    async getSession() {
...
describe('drop-copy stream drops when the product is not on the key', () => {
  ...
  function upgradeStatus(host: string, port: number, path: string, product?: string): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      if (product !== undefined) headers['x-product'] = product;
  ...
  it('listed product upgrades; foreign product and missing product refuse with 401', async () => {
  ...
  it('empty list stays open without an x-product header', async () => {
  ...
  it('session token (no kid) is not product-gated', async () => {
    ...
    expect(await upgradeStatus(host, port, `${DROP_COPY_STREAM_PATH}?access_token=${issued.token}`, FOREIGN)).toBe(101);
  });

  it('foreign product never receives a ready frame', async () => {
    const { port } = await boot(keyPort([LISTED]));
    const token = await access();
    const frames: string[] = [];
    const ws = new WebSocket(`ws://127.0.0.1:${port}${DROP_COPY_STREAM_PATH}?access_token=${token}`, {
      headers: { 'x-product': FOREIGN },
    });
