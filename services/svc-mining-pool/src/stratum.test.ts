...
describe('Stratum share protocol', () => {
  it('accepts a valid share and rejects malformed requests by named reason', () => {
    ...
    expect(acceptShare({ minerId: 'm', worker: 'w', jobId: 'j', nonce: 'abcd', shareHash: '00ff', target: '00ff' })).toMatchObject({ accepted: true });
    expect(acceptShare({ minerId: 'm', worker: 'w', jobId: '', nonce: 'abcd', shareHash: '00ff', target: '00ff' })).toEqual({ accepted: false, reason: 'job_not_found' });
  });
});
