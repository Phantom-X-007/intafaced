...
export const FEATURES = [
  ...
  f('bank.business', 'svc-bank-biz — corporate accounts, maker/checker, expense cards, invoicing, crypto payroll (§31)', {
    ...
    note: 'W10 L08 PARTIAL: maker/checker dual-control for over-threshold transfers on corporate accounts in svc-bank (no separate svc-bank-biz yet). KYB/payroll/invoicing/expense cards residual or §13. Law §31:811, gap-closed 2026-08-08 — AND THE LAW WROTE A TEST FOR THIS SERVICE BEFORE THE SERVICE EXISTED. §38:854 adds "payroll run integrity test (multi-recipient atomicity)" to the DoD. There is no `services/svc-bank-biz`, so that DoD clause has had nothing to attach to since the day it was written, and a DoD condition with no capability under it is a condition nobody can fail — which is the whole reason this row now exists. §31:811 phases it 5; §38:854 gives the plane as F with P recipients (payroll to sovereign vaults), so `plane: F` is stated rather than defaulted: the corporate account is custodial, the payee may not be. Scope per §31:811: corporate accounts on the same ledger, multi-user roles with maker/checker and spend thresholds, team expense cards with per-card limits and category locks, invoicing wired to Pay, accounting exports, and CRYPTO PAYROLL — scheduled multi-recipient payouts in a stables/fiat mix, contractor self-onboarding, payslip records. Blocked on pay.gateway (`ready`, not `done`) because §31:811 wires invoicing to Pay; bank.accounts and bank.cards are `done` and are the halves that already exist to build on. THE ATOMICITY RULE, WRITTEN NOW RATHER THAN DISCOVERED IN PRODUCTION: a payroll run that pays 40 of 50 recipients and reports success is exactly the failure §38:854 names. Under §0.6 the run is one ledger movement or it is refused — no per-recipient partial state a retry can double-pay — and the DoD test asserts that, not merely that a payout endpoint returned 200. NOT ON THE BOARD AT ALL, and this row cannot be `done` without it: §31:811 says "KYB per Lane B", and there is no KYB anywhere in the platform. identity.kyc is individual KYC, and a corporate account holder is not a person. Naming that here so whoever claims this does not discover it halfway.',
  }),
