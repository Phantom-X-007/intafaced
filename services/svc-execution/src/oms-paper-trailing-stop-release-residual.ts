/**
 * Residual-release leftover already on an expired paper trailing-stop parent.
 * Hands the retained leftover through ledger-client. This door never invents an
 * amount from trail or the clock, never invents a live venue, and does
 * not touch matching.
 */
import { formatAmount, parseAmount } from '@intafaced/ledger-client';
import type { PaperGate } from './oms-paper.js';

export type OmsPaperTrailingStopReleaseResidualRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'not_expired'
  | 'already_released'
...
export type OmsPaperTrailingStopReleaseResidualRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperTrailingStopReleaseResidualRefuseReason;
  readonly detail: string;
};

export type OmsPaperTrailingStopReleaseResidualOk = {
  readonly ok: true;
...
export type OmsPaperTrailingStopReleaseResidualResult =
  | OmsPaperTrailingStopReleaseResidualOk
  | OmsPaperTrailingStopReleaseResidualRefusal;

function refuse(
  reason: OmsPaperTrailingStopReleaseResidualRefuseReason,
  detail: string,
): OmsPaperTrailingStopReleaseResidualRefusal {
...
): { ok: true; text: string } | OmsPaperTrailingStopReleaseResidualRefusal {
  if (raw === null || raw === undefined) {
    return refuse(
      'missing_residual',
      'residual.remaining is missing — refusing to invent leftover from trail or the clock',
    );
  }
  const text = raw.trim();
  if (text.length === 0) {
...
/**
 * Residual-release leftover already retained on an expired paper trailing-stop parent through ledger-client.
 */
export function releaseExpiredPaperTrailingStopResidual(input: {
  ...
  paper?: PaperGate;
}): OmsPaperTrailingStopReleaseResidualResult {
