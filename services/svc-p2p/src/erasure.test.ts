...
if (!available) {
  describe.skip('p2p erasure (Postgres unavailable — start docker compose)', () => {
    it('skipped', () => undefined);
  });
} else {
  const sql = postgres(URL, {
    max: 8,
    connection: { search_path: 'p2p,public', application_name: 'svc-p2p-erasure-test' },
    onnotice: () => undefined,
  });

  await assertTestDatabase(sql, 'svc-p2p erasure');

  // Same advisory lock, same reason, as the other two Postgres suites here:
  // both migrations re-assert their CHECK constraints with DROP … IF EXISTS
  // first, so two processes bringing the schema up at once take the same table
  // locks in opposite orders and Postgres kills one with a deadlock.
