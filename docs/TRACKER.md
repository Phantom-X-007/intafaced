...
| 🔨 | WebSocket fan-out: depth, trades, orders, positions <br/>_Updated 2026-07-31 residual-ws: four fan-out *channels* exist — public depth (ws.depth), public trade tape, private orders+fills (JWT /private/stream), private positions channel (#227). Positions stream is **empty-honest**: ready frame only until trade.futures publishes positionUpdated (source+hub tests prove no invented rows). REST GET /positions also []. Status stays **wip** because title product "positions" means live futures events, not only a silent channel. Do not mark done without trade.futures or a doctrine change that empty-honest channels complete the title._ | F |  | `ws.gateway` |
