...
### B1 · KYB money gate (shape only; wiring is later)

After scopes exist, money procedures that today call `assertMerchantActive` (status) must also require the owner-sealed KYB predicate before value movement. **That wiring is D26-P1-P10 / gateway residual**, not this ADR’s code — and must not invent the predicate.

Open PSP/KYB product PRs may own dossier history and PSP mode; this ADR does not dual-edit those files.
...
### D26-P1-P10 (later — after Nitro seals A2)

1. Published grant law read by `assertMerchantPayScopeGrantAllowed` (or successor).
2. Real issuance path under that law.
3. KYB money gate wired per sealed predicate — still no invent.

---

## What agents may implement without asking again

- Call-site wiring that **invokes** `assertMerchantPayScopeGrantAllowed` and surfaces the refuse.
- Tests that pin “no session `pay:*`” and “grant refuses when law unpublished.”
- KYB **money-gate** wiring **only after** the owner predicate is published (still not inventing scopes).

## What still needs the owner

- Every answer in **A2**.
- Any production claim that merchants can authenticate.
