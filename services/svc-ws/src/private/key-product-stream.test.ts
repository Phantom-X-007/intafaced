...
describe('private stream drops when the product is not on the key', () => {
  let server: Server;
  ...
  it('session token (no kid) is not product-gated', async () => {
    ...
    expect(await upgradeStatus(host, port, `${PRIVATE_STREAM_PATH}?access_token=${issued.token}`, FOREIGN)).toBe(101);
  });
