...
| 🟢 | CCXT-compatible public API (bots + terminals connect) <br/>_partial — markets+orderbook public REST live on svc-trade (/api/v1/markets, /api/v1/orderbook/:symbol) + edge preservePath /api/v1 → TRADE_URL; private routes still open_ | F |  | `trade.ccxt-api` |
