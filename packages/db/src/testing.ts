...
export async function createTestDb(options: TestDbOptions): Promise<TestDb> {
  const url = options.url ?? process.env.TEST_DATABASE_URL ?? 'postgres://intafaced_ops:intafaced_ops@localhost:5432/intafaced';
...
export async function postgresAvailable(url?: string): Promise<boolean> {
  const target = url ?? process.env.TEST_DATABASE_URL ?? 'postgres://intafaced_ops:intafaced_ops@localhost:5432/intafaced';
