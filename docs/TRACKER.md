...
| 🟢 | CCXT-compatible public API (bots + terminals connect) <br/>_partial — public REST on svc-trade: /api/v1/markets, /api/v1/orderbook/:symbol, /api/v1/ticker/:symbol (BBO+last), /api/v1/trades/:symbol (public tape from taker fills, no user ids); edge preservePath /api/v1 → TRADE_URL; private routes still open_ | F |  | `trade.ccxt-api` |
