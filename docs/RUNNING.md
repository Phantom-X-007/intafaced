...
Tip-image law (D26-P2-04): [`FLEET-TIP-IMAGES.md`](ops/FLEET-TIP-IMAGES.md) — local `--build` (and `--no-cache` when layers lie), staging digest + `--no-build`, never Hub-pull `intafaced/app:dev`. Vendor jars are a **separate** artifact: gitignored `target/*.jar` older than D-S-17 neutering are the known lie — rebuild, do not commit.
