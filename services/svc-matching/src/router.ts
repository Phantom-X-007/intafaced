...
import type { EngineAmend, EngineOrder } from './engine/types.js';
import { bindPostOnlyTif } from './engine/post-only.js';
...
export const massCancelBodySchema = z.object({
  /** §5.1: account ids only. This service never learns whose account it is. */
  accountId: z.string().min(1).max(128),
...
export function toEngineOrder(body: z.infer<typeof submitBodySchema>): EngineOrder {
...
export function toEngineAmend(orderId: string, body: z.infer<typeof amendBodySchema>): EngineAmend {
  return {
    orderId,
...
export { registerRoutes, MatchingAuthError, MatchingForbiddenError } from './router-register.js';
export type { MatchingRouteOptions } from './router-register.js';
