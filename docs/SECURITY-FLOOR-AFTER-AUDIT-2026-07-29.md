...
**Product trust floor remains:** [`PEACE-OF-MIND-AUDIT-CURRENT.md`](./PEACE-OF-MIND-AUDIT-CURRENT.md).

| Field | Value |
| --- | --- |
| **Verified against** | `origin/main` tip **`4311cff`** (includes #80, #81, #82) |
...
| Living product scoreboard | `docs/PEACE-OF-MIND-AUDIT-CURRENT.md` on main (tip note may still say `88e5e33`; actual tip is `4311cff` after #82 docs-only) |
...
| **Attack readiness / Track B** | **Map now · run later** (phase **advanced**: stack *can* listen via `platform:up`) | Strix still **PARKED** until deliberate non-prod campaign + explicit Nitro go |
...
| Research “NOW: routers unmounted” | **Wrong** on live main — routers mounted; near-phase attack tools are *available*, not *due* |
...
| PEACE tip SHA | Update to `4311cff` when next docs touch |
...
2. **Doctrine machine growth** — either Semgrep CE custom rules *or* tighter home greps in `tooling/ci` for money-as-number / bare `ledger.post` / cross-service SQL (encode §0, don’t wait for Strix).  
...
5. `PEACE-OF-MIND`: tip SHA → `4311cff`; residual list already correct enough (L2-6 + go-live).  
