/**
 * Operator kill-switch mutate dual-control. Missing/same confirm refuses.
 * Matching halt and ledger freeze already require two distinct operators;
 * one-operator kill at this door is the same lie as one-operator halt.
 * Confirmer is a named identity on the body — the edge does not invent a second caller.
 */
...
export function dualControlRefuse(
  ...
  if (actor === null) {
    return {
      ...
      message: 'operator identity is required; the edge does not invent a caller',
    };
  }
  if (confirmOperatorId === null) {
    return {
      code: MISSING_OPERATOR,
      message: 'confirming operator identity is required; the edge does not invent a second caller',
    };
  }
  if (confirmOperatorId === actor) {
    return {
      code: MISSING_OPERATOR,
      message: 'confirming operator must be a distinct identity; the edge does not invent a second caller',
    };
  }
  return null;
}
