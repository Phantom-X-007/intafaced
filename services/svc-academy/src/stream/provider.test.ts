...
describe('LiveKitStreamProvider', () => {
  it('uses the RoomService HTTP API and signs a real join token', async () => {
    ...
    const provider = new LiveKitStreamProvider({
      url: 'wss://livekit.example', apiKey: 'API123', apiSecret: 'secret', tokenTtlSeconds: 600,
      fetch: async (url, init) => {
        requests.push({ url: String(url), body: String(init?.body), authorization: String(new Headers(init?.headers).get('authorization')) });
    ...
    expect(requests[0]).toMatchObject({ url: 'https://livekit.example/twirp/livekit.RoomService/CreateRoom', body: '{"name":"session-1"}' });
    ...
    const payload = JSON.parse(Buffer.from(credential.token.split('.')[1]!, 'base64url').toString()) as { sub: string; video: { roomJoin: boolean; room: string; canPublish: boolean } };
