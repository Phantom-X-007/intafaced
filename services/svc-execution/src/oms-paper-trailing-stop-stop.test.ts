...
import { stopPaperTrailingStopParent } from './oms-paper-trailing-stop-stop.js';

const PAPER_ON = { enabled: true } as const;
const PAPER_OFF = { enabled: false } as const;

describe('stopPaperTrailingStopParent', () => {
  it('refuses missing / whitespace parentClientOrderId', () => {
    expect(
      stopPaperTrailingStopParent({
        status: 'paper',
        remaining: '10',
        paper: PAPER_ON,
      }),
  ...
  it("happy: status paper + remaining '10' + paper on", () => {
    const stopped = stopPaperTrailingStopParent({
      parentClientOrderId: 'p-trail',
      status: 'paper',
      remaining: '10',
      paper: PAPER_ON,
