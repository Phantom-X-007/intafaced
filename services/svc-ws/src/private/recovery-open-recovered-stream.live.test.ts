...
describe('private stream opens on the recovered session after a recovery redeem', () => {
  let server: Server;
  ...
  async function access(): Promise<string> {
    const issued = await issueAccessToken({ userId: USER, sessionId: KEEP, scopes: ['trade:write'] }, tokens);
    return issued.token;
  }
  ...
  it('spent or missing recovery code refuses before the recovered stream opens', () => {
    expect(() => recoveryCodeOpensRecoveredStream({ code: '', recoveryCodeHashes: ['h'] })).toThrow(/missing/);
    expect(() => recoveryCodeOpensRecoveredStream({ code: CODE, recoveryCodeHashes: [] })).toThrow(/spent/);
  });

  it('recovered session upgrades after a remaining recovery code', async () => {
    recoveryCodeOpensRecoveredStream({ code: CODE, recoveryCodeHashes: ['h'] });
