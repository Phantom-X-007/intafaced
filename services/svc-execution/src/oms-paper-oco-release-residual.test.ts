...
import { releaseExpiredPaperOcoResidual } from './oms-paper-oco-release-residual.js';

const PAPER_ON = { enabled: true } as const;
const PAPER_OFF = { enabled: false } as const;
const RETAINED_REMAINING = '1.25';

describe('releaseExpiredPaperOcoResidual', () => {
  it('refuses missing parentClientOrderId', () => {
    expect(
      releaseExpiredPaperOcoResidual({
        status: 'expired',
  ...
  it("refuses status paper / running / omitted with not_expired even when remaining is '1.25'", () => {
    ...
    expect(releaseExpiredPaperOcoResidual({ ...base, status: 'paper' })).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
    expect(releaseExpiredPaperOcoResidual({ ...base, status: 'running' })).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
    expect(releaseExpiredPaperOcoResidual(base)).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
  });
  ...
  it('refuses omitted / null / whitespace remaining with missing_residual', () => {
    ...
    expect(releaseExpiredPaperOcoResidual({ ...base, remaining: null })).toMatchObject({
      ok: false,
      reason: 'missing_residual',
    });
    expect(releaseExpiredPaperOcoResidual({ ...base, remaining: '' })).toMatchObject({
      ok: false,
      reason: 'missing_residual',
    });
    expect(releaseExpiredPaperOcoResidual({ ...base, remaining: '   ' })).toMatchObject({
  ...
  it("happy expired paper remaining '1.25' + paper on: released through ledger-client", () => {
    const result = releaseExpiredPaperOcoResidual({
      parentClientOrderId: 'p-oco',
      kind: 'oco',
      status: 'expired',
      remaining: RETAINED_REMAINING,
