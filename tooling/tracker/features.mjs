...
export const FEATURES = [
  ...
  f('ws.gateway', 'WebSocket fan-out: depth, trades, orders, positions', {
    ...
    requires: ['services/svc-ws', 'packages/market-data', 'packages/events'],
    note: 'Four streams mounted. Public: depth + TRADE tape on /stream. Private JWT /private/stream: orders (orderUpdated), fills (fillSettled), positions (positionUpdated) — ready frames for all three on connect; owner-isolated fan-out; no invented position rows. Live position payloads arrive when trade.futures publishes positionUpdated (catalog subject intafaced.trade.position.updated); until then the channel stays silent, matching REST GET /positions → [].',
  }),
