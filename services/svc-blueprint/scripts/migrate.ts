 ...
 * Migration runner.
 *
 * Applies every `NNNN_*.sql` in order, tracked in `blueprint.__migrations` so a
 * re-run is a no-op. Pass `--down` to apply reversals in reverse order — which
 * exists so CI can prove the migrations are reversible (§14), not because
 * anyone should run it against a database holding real Blueprints.
 */
 ...
 * strays outside `blueprint` fails here rather than in production.
 */
const url = process.env.DATABASE_URL ?? 'postgres://svc_blueprint:svc_blueprint@localhost:5433/intafaced';
...
async function main() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS blueprint.__migrations (
  ...
  if (down) {
    for (const up of [...ups].reverse()) {
      ...
      await sql`DELETE FROM blueprint.__migrations WHERE name = ${up}`;
    }
    console.log('✓ reversed');
    return;
  }

  const applied = new Set((await sql<Array<{ name: string }>>`SELECT name FROM blueprint.__migrations`).map((r) => r.name));
  ...
  for (const file of ups) {
    ...
    await sql`INSERT INTO blueprint.__migrations (name) VALUES (${file})`;
  }
