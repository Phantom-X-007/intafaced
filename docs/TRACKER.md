...
| 🟢 | Internal market-maker seeding books at launch <br/>_Updated 2026-07-31 residual: seedMarket + job OFF + marketMakerMakerFill + settleFill house-MM maker branch (inline). Still missing: orderFilled event accountId recovery, cancel/reseed lifecycle, live mid oracle. Not done._ | F |  | `trade.mm-bot` |
...
| 🟡 | apps/admin — kill-switches live via svc-edge; residual product surface <br/>_#186 wired edge control plane; A-P5-OPS: ops runbook + `/admin/status` + admin board posts live module kills when EDGE_URL + ADMIN_OPERATOR_TOKEN set. Still residual: listings/fees/treasury product, flag store, operator SSO, reconcile stub, ledger freeze UI still partly stubbed. See docs/OPS-KILL-SWITCH-RUNBOOK.md._ | F |  | `ops.admin` |
