 ...
 * Migration runner.
 *
 * Applies every `NNNN_*.sql` in order, tracked in `agents.__migrations` so a
 ...
 * should run it against a database holding a real audit trail.
 */
 ...
 * strays outside `agents` fails here rather than in production.
 */
const url = process.env.DATABASE_URL ?? 'postgres://svc_agents:svc_agents@localhost:5433/intafaced';
...
async function main() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS agents.__migrations (
  ...
  if (down) {
    for (const up of [...ups].reverse()) {
      ...
      await sql`DELETE FROM agents.__migrations WHERE name = ${up}`;
    }
    console.log('✓ reversed');
    return;
  }

  const applied = new Set((await sql<Array<{ name: string }>>`SELECT name FROM agents.__migrations`).map((r) => r.name));
  ...
  for (const file of ups) {
    ...
    await sql`INSERT INTO agents.__migrations (name) VALUES (${file})`;
  }
