...
### 1.2 What is *not* the problem (ruled out with evidence)
...
| Need for Nitro to babysit git | Operator mode already exists; issue is *which truth file agents trust* |
...
| What should *this campaign* ship next? | `docs/BOARD-CLEAR-NEXT.md` | same (correct for campaign) |
| Who is coding *this hour* on which paths? | `docs/LIVE-LANES.md` + open PRs | memory / nothing |
...
**Denon’s sentence maps mainly to F1, F3, F4.** F5 is already engineered. F6–F7 are self-inflicted process debt that *cause* F1–F4.
...
**Thrift verdict:** contributor to *when* honesty is delayed, **not** root of dual-build. Killing thrift would not fix C1–C5 and would **hurt** speed/budget.
...
This satisfies Denon’s *intent* (truth DB for product context + less conflict) without literal micro-PR registry spam.
...
## Phase 7 — Execution plan for *later* chats (not this turn)
