...
| `socket.social-recovery`          | **CLOSED 2026-08-18 (S-A1).** `UserElectedRecovery` is a user-elected M-of-N ERC-1271 owner. Proven: `createAccount(recovery)` sets `SmartAccount.owner` to it; factory still takes the key it is given (not default). Platform is never a guardian. Unaudited |
