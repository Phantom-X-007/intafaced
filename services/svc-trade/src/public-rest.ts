...
export interface PublicRestDeps {
   ...
   * Optional untilMs (unix ms) filters fills.ts <= until in SQL.
   * Optional side filters fills.side in SQL (taker prints only).
   */
  publicTape(marketId: string, limit: number, sinceMs?: number, side?: 'buy' | 'sell', untilMs?: number): Promise<PublicTapePrint[]>;
  /**
...
/** Optional quote-asset filter. Absent → every quote. */
export function parseMarketQuote(raw: unknown): { ok: true; quote?: string } | { ok: false; message: string } {
  if (raw === undefined || raw === null || raw === '') return { ok: true, quote: undefined };
  if (typeof raw !== 'string') return { ok: false, message: 'quote must be 1-32 characters' };
  const quote = raw.trim();
  if (quote.length < 1 || quote.length > 32) return { ok: false, message: 'quote must be 1-32 characters' };
...
export function registerPublicRest(app: FastifyInstance, deps: PublicRestDeps): void {
  ...
  app.get<{ Querystring: { status?: string; kind?: string; quote?: string; base?: string } }>('/api/v1/markets', async (req, reply) => {
    ...
    const quoteParsed = parseMarketQuote(req.query.quote);
    if (!quoteParsed.ok) {
  ...
  app.get<{ Params: { symbol: string }; Querystring: { limit?: string; since?: string; until?: string; side?: string } }>(
    ...
    async (req, reply) => {
      ...
      // since / until / side → SQL on fills via publicTape — not a post-filter of a mixed page.
      const tape = await deps.publicTape(market.id, limit, sinceParsed.sinceMs, sideParsed.side, untilParsed.untilMs);
