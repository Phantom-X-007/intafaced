...
test('Token LOOK keeps money reads first and leaves the behavior script byte-identical', async () => {
  ...
  expect(createHash('sha256').update(currentScript).digest('hex')).toBe(
    createHash('sha256').update(scriptFrom(baseline)).digest('hex'),
  );
});
...
for (const width of [1440, 390]) {
  test(`Token access leads; service actions stay disclosed at ${width}`, async ({ page, browser }) => {
    ...
    await page.evaluate(() => document.fonts.ready);
    await page.screenshot({ animations: 'disabled', path: join(repoRoot, 'tooling/uiproof/crops/look-token-access', `${width}-empty.png`) });
    ...
    await page.screenshot({ animations: 'disabled', path: join(repoRoot, 'tooling/uiproof/crops/look-token-access', `${width}-stake.png`) });
    ...
    await page.evaluate(() => window.scrollBy(0, -64));
    await page.screenshot({ animations: 'disabled', path: join(repoRoot, 'tooling/uiproof/crops/look-token-access', `${width}-operations.png`) });
    ...
    writeFileSync(join(evidenceDir, `${width}.json`), JSON.stringify({
      ...
      scriptSha256: createHash('sha256').update(scriptFrom(readFileSync(tokenFile, 'utf8'))).digest('hex'),
      server: JSON.parse(readFileSync(join(repoRoot, '.artifacts/uiproof/provenance.json'), 'utf8')),
      route: '/token', browser: `Chromium ${browser.version()}`, viewport: { width, height },
      ...
      evidence: ['down', 'empty', 'stake', 'operations'].map(state => ({
        ...
        sha256: createHash('sha256').update(readFileSync(join(evidenceDir, `${width}-${state}.png`))).digest('hex'),
        fixture: state === 'down' ? 'All API reads HTTP 503' : 'listStakes HTTP 200 []; all other API reads HTTP 503; no seeded money, tier, or discount',
      })),
      writes,
    }, null, 2) + '\n');
  });
