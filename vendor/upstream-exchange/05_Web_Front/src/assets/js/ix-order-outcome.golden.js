...
assert.notStrictEqual(trade.indexOf("require('./ix-gtd-ticket.js')"), -1, 'ix-trade must install the Bazaar GTD ticket');
var installer = fs.readFileSync(path.join(__dirname, 'ix-gtd-ticket.js'), 'utf8');
assert.notStrictEqual(installer.indexOf('function assertTicketExpire'), -1, 'installer missing GTD expire refuse');
assert.notStrictEqual(installer.indexOf("tif !== 'GTD' && tif !== 'GTT'"), -1, 'installer must refuse GTD/GTT without expireAt');
assert.notStrictEqual(installer.indexOf('the engine does not invent one'), -1, 'installer must not invent expiry');
