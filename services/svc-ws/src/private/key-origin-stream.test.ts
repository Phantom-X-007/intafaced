...
describe('private stream drops when the Origin is not on the key', () => {
  let server: Server;
  ...
  it('session token (no kid) is not origin-gated', async () => {
    ...
    expect(await upgradeStatus(host, port, `${PRIVATE_STREAM_PATH}?access_token=${issued.token}`, FOREIGN_ORIGIN)).toBe(101);
  });
