...
| `GET /admin/status`                        | Who is killed, **what cannot be killed** (`outsideTheDoor`), **kill durability** (`killState` — process-local; `multiReplicaShared: false`), **`flagEdgeGateway.enforced: false`**, plus **ops honesty**: `networkSignal` (unset≠clear), `freezeAuthority` (only `ledger.posting`; invent trade/pay freeze refused), `complianceQueue` (empty-safe; partner absent named), `analytics` (warehouse dark/unavailable; never live cubes without lag). |
| `GET /admin/compliance/queue`              | In-memory screening/review queue snapshot — honest empty; no invent cases. Mechanism only (full case product residual).                                                                                                                                                                                                                                                                                                                            |
| `POST /admin/compliance/queue/disposition` | Dispose a case. `partner_cleared` **refuses** when screening partner absent (`409 refuse.partner_absent`).                                                                                                                                                                                                                                                                                                                                         |
...
- **VPN/Tor partner product + request probe.** Mechanism on status (`networkSignal` unset/dark/clear/flagged + fail-closed env). Partner procurement and list content remain Class X — no vendor invent here.
- **Full compliance case-management product.** In-memory queue + partner_cleared refuse is mechanism only; durable UI/DB/SLA residual.
- **Analytics ETL/lag probe process.** Status names dark/unavailable warehouse; physical replica probe + cube jobs stay residual (contracts Stage-1).
