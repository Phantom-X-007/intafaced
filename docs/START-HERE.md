...
- **Agents find code via Graphify** (not by re-reading the repo): `graphify query "…"`. First time in a worktree: `pnpm graphify:extract`. You do not run this. Later check: say **graphify peace** — [`GRAPHIFY-PEACE-CHECK.md`](GRAPHIFY-PEACE-CHECK.md).
