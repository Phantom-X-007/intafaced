...
const targetsInput = z.object({ channel: outOfAppChannelSchema.optional() }).optional();
  ...
  return router({
    ...
    notify: router({
      ...
      targets: scopedProcedure('notify:read', { module: 'notify' })
        ...
        .query(async ({ ctx, input }) => {
          const targets = await notify.listTargets(ctx.principal.userId, input?.channel);
          return targets
      ...
      deliveries: scopedProcedure('notify:read', { module: 'notify' })
        ...
        .query(async ({ ctx, input }) =>
          (await notify.deliveriesFor(ctx.principal.userId, input.notificationId, input.channel, input.status)).map(deliveryToWire),
        ),
      ...
      alerts: scopedProcedure('notify:read', { module: 'notify' })
        .input(
          z
            .object({
              status: z.enum(['active', 'fired', 'cancelled']).optional(),
        ...
        .query(async ({ ctx, input }) => {
          ...
          return {
            items: (await alerts.list(ctx.principal.userId, input?.status, input?.marketId)).map(priceAlertToWire),
