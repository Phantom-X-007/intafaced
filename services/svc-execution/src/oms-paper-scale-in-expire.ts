/**
 * Expire one live paper scale-in parent when its window ends.
...
export type OmsPaperScaleInExpireRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'already_expired'
  | 'already_stopped'
...
export type OmsPaperScaleInExpireRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperScaleInExpireRefuseReason;
  readonly detail: string;
};

export type OmsPaperScaleInExpireOk = {
  readonly ok: true;
...
export type OmsPaperScaleInExpireResult =
  | OmsPaperScaleInExpireOk
  | OmsPaperScaleInExpireRefusal;

function refuse(
  reason: OmsPaperScaleInExpireRefuseReason,
  detail: string,
): OmsPaperScaleInExpireRefusal {
...
/**
 * Expire a paper scale-in parent using expireAt already retained on the window.
 * Residual is not released or consumed. Paper off refuses a live venue.
 */
export function expirePaperScaleInParent(input: {
