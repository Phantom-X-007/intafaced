...
import { releaseExpiredPaperPeggedResidual } from './oms-paper-pegged-release-residual.js';

const PAPER_ON = { enabled: true } as const;
const PAPER_OFF = { enabled: false } as const;
const RETAINED_REMAINING = '1.25';

describe('releaseExpiredPaperPeggedResidual', () => {
  it('refuses missing parentClientOrderId', () => {
    expect(
      releaseExpiredPaperPeggedResidual({
        status: 'expired',
  ...
  it("refuses status paper / running / omitted with not_expired even when remaining is '1.25'", () => {
    ...
    expect(releaseExpiredPaperPeggedResidual({ ...base, status: 'paper' })).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
    expect(releaseExpiredPaperPeggedResidual({ ...base, status: 'running' })).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
    expect(releaseExpiredPaperPeggedResidual(base)).toMatchObject({
      ok: false,
      reason: 'not_expired',
    });
  });
  ...
  it('refuses omitted / null / whitespace remaining with missing_residual', () => {
    ...
    expect(releaseExpiredPaperPeggedResidual({ ...base, remaining: null })).toMatchObject({
      ok: false,
      reason: 'missing_residual',
    });
    expect(releaseExpiredPaperPeggedResidual({ ...base, remaining: '' })).toMatchObject({
      ok: false,
      reason: 'missing_residual',
    });
    expect(releaseExpiredPaperPeggedResidual({ ...base, remaining: '   ' })).toMatchObject({
  ...
  it("happy expired paper remaining '1.25' + paper on: released through ledger-client", () => {
    const result = releaseExpiredPaperPeggedResidual({
      parentClientOrderId: 'p-peg',
      kind: 'pegged',
      status: 'expired',
      remaining: RETAINED_REMAINING,
