 ...
 *
 * §2: services never touch another service's tables. Every value movement in a
 * raise — escrowing supply, taking a commitment, settling a contributor,
 * refunding a failure, releasing a vesting tranche — goes over this client into
 * svc-ledger, which owns the balance graph (Doctrine §0.6). svc-launch holds no
 * balance of its own and has no code path that could.
 *
 * It implements the same `LedgerClient` interface as the in-memory reference,
 * so every money path in this service is unit-tested against `MemoryLedger`
 * without a network — and the conformance suite (§4.4) is what makes that
...
export function createLedgerClient(baseUrl: string, internalSecret: string): LedgerClient {
   ...
   */
  const authHeaders = () => serviceAuthHeaders('svc-launch', internalSecret);
  ...
  async function call<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      // Preserve the ledger's own error text. 'ledger.insufficient_funds' on a
      // commitment is a contributor without the money; 'ledger.frozen' is a
      // platform-wide halt, and every raise in the book is stopped. Collapsing
      // them into "request failed" makes both unactionable.
