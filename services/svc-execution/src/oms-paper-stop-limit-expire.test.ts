...
import { expirePaperStopLimitParent } from './oms-paper-stop-limit-expire.js';
...
describe('expirePaperStopLimitParent', () => {
  it('refuses missing / whitespace parentClientOrderId', () => {
    expect(
      expirePaperStopLimitParent({
        status: 'paper',
        expireAt: EXPIRE_AT,
        remaining: '10',
        paper: PAPER_ON,
  ...
  it("happy: status paper + expireAt + remaining '10' + paper on", () => {
    const expired = expirePaperStopLimitParent({
      parentClientOrderId: 'p-stpl',
      status: 'paper',
      expireAt: EXPIRE_AT,
      remaining: '10',
