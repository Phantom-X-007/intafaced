...
**Context:** direction given — *"the bizzan ui and app is better built so we use that ui, add all of our other features to it."*
...
| `jquery` | 3.3.1 | loaded *alongside* Vue |
...
The value is the *product surface*: the flows, the layouts, the states, the copy. None of that is Vue-specific. What we would inherit by adopting the stack is a framework with no security patches, a retired HTTP client, jQuery, a build that will not run on a current Node, and a realtime layer welded to Spring.
...
That is a real amount of work whose *output* is a 2018 front-end. The same effort spent porting screens into `apps/web` produces the same screens on a stack that gets patched.
