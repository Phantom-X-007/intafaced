...
export const FEATURES = [
  ...
  f('ws.gateway', 'WebSocket fan-out: depth, trades, orders, positions', {
    ...
    requires: ['services/svc-ws', 'packages/market-data'],
    note: 'Still wip on main. Depth done + public TRADE tape + private orders/fills on /private/stream (orderUpdated+fillSettled, JWT). Fourth stream (positions / positionUpdated) is only on open PR #227 — not merged; title names four streams; three of four is not done.',
  }),
  ...
  f('pay.rails', 'RailAdapter interface + crypto-native + card-sandbox', {
    ...
    phase: '3',
    status: 'ready',
    // Not blocked on pay.gateway: rails are the value-move adapters; hosted
    // checkout / merchant branding is a separate surface. Live EVM rail is
    // open PR #226 only — must not read as done on main.
    dependsOn: ['ledger.double-entry'],
    requires: ['services/svc-pay/src/rails'],
    note: 'Updated 2026-07-31 honesty re-pass: still `ready` not `done` on main. Interface + conformance kit are real and tested; NEITHER v1 RAIL ON MAIN MOVES REAL VALUE and the code now says so. Every adapter declares `mode: live|sandbox` (conformance kit refuses one that does not); crypto-native derives its mode from the chain port, so MemoryChain makes it a sandbox whatever §13 says. Two gates in rails/posture.ts: APP_ENV staging/prod REFUSES TO BOOT with any sandbox rail registered unless PAY_ALLOW_SANDBOX_RAILS=true, and payout/refund re-check at the call site before the ledger moves (pay.rail_not_live -> SERVICE_UNAVAILABLE). Production chain default is UnconfiguredChain, which refuses every call instead of returning a fabricated txHash. /ready and railHealth carry mode. Live EVM crypto rail (EvmLiveChain, watcher, HD accept, hot-wallet payout) exists only on open PR #226 — not merged; do not mark rails done from that PR until it lands.',
  }),
  f('pay.user-money', 'User deposit + withdrawal — the two paths off the merchant path', {
    ...
    requires: ['services/svc-pay'],
    note: 'Updated 2026-07-31 honesty re-pass: still not `done` on main. deposit.credit (admin:treasury) and withdrawal.* (trade:withdraw INTERACTIVE_ONLY + 2FA / trade:read) are reachable on mounted /trpc via svc-edge /api/pay — edge-signed principal required (router.mount.test.ts). Double-submit proven under CONCURRENCY: two identical withdrawals in flight debit once and carry ONE rail idempotency key; a redelivered deposit credits once; two concurrent affordable-alone withdrawals cannot overdraw. Money-path suite against real Postgres on intafaced_test. Depends on pay.rails, which has no live rail on main (#226 open only) — a sandbox payout is refused before anything moves rather than fabricating a `sent`.',
  }),
  ...
  f('ops.admin', 'apps/admin — listings, fee params, treasury, kill-switches', {
    ...
    dependsOn: ['infra.ui-tokens'],
    requires: ['apps/admin'],
    note: '2026-07-31 honesty re-pass: still `ready` not `done`. #230 accidentally pasted launch.token-factory text onto this row — stripped. What is real: #186 wired svc-edge `/admin/kill-switches` + `/admin/ledger/*`, and apps/admin has `control-plane-client` plus `/api/kill-switch` + `/api/ledger-freeze` that call edge when ADMIN_OPERATOR_TOKEN / ADMIN_TREASURY_TOKEN are set. What is NOT done: the kill-switch board UI still stages flag overrides in browser useState only (does not call those routes); the ledger page still uses `operator-commands` stubs that simulate freeze/reconcile; ZERO test files under apps/admin; no operator SSO; title items listings/fee params are not product surfaces. An operator console that only stages locally is not a control surface.',
  }),
