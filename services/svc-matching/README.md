...
| `POST /session/dead`                         | `{ sessionId }` · **service-only**                                    | `{ accepted, sessionId, cancellations[], rejected }` · cancel-on-disconnect; tagged rests cancel; new tagged submits refuse; missing session refuses                                                                                              |
