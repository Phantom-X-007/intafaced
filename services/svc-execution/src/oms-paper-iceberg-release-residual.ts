/**
 * Release leftover residual already on an expired paper iceberg parent.
 * Hands the retained leftover through ledger-client. This door never invents an
 * amount from display qty or the clock, never invents a live venue, and does
 * not touch matching.
 */
import { formatAmount, parseAmount } from '@intafaced/ledger-client';
import type { PaperGate } from './oms-paper.js';

export type OmsPaperIcebergReleaseResidualRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'not_expired'
  | 'already_released'
...
export type OmsPaperIcebergReleaseResidualRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperIcebergReleaseResidualRefuseReason;
  readonly detail: string;
};

export type OmsPaperIcebergReleaseResidualOk = {
  readonly ok: true;
...
export type OmsPaperIcebergReleaseResidualResult =
  | OmsPaperIcebergReleaseResidualOk
  | OmsPaperIcebergReleaseResidualRefusal;

function refuse(
  reason: OmsPaperIcebergReleaseResidualRefuseReason,
  detail: string,
): OmsPaperIcebergReleaseResidualRefusal {
...
): { ok: true; text: string } | OmsPaperIcebergReleaseResidualRefusal {
  if (raw === null || raw === undefined) {
    return refuse(
      'missing_residual',
      'residual.remaining is missing — refusing to invent leftover from display qty or the clock',
    );
  }
  const text = raw.trim();
  if (text.length === 0) {
...
/**
 * Release leftover already retained on an expired paper iceberg parent through ledger-client.
 */
export function releaseExpiredPaperIcebergResidual(input: {
  ...
  paper?: PaperGate;
}): OmsPaperIcebergReleaseResidualResult {
