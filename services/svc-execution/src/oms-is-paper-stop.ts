/**
 * Stop one running paper implementation-shortfall parent.
...
export type OmsIsPaperStopRefuseReason =
  | 'missing_parent'
  | 'not_running'
  | 'already_stopped'
  | 'not_live'
...
export type OmsIsPaperStopRefusal = {
  readonly ok: false;
  readonly reason: OmsIsPaperStopRefuseReason;
  readonly detail: string;
};

export type OmsIsPaperStopOk = {
  readonly ok: true;
...
export type OmsIsPaperStopResult = OmsIsPaperStopOk | OmsIsPaperStopRefusal;

function refuse(reason: OmsIsPaperStopRefuseReason, detail: string): OmsIsPaperStopRefusal {
...
/**
 * Stop a running paper IS parent. Existing children stay as recorded.
 * Residual is not released or consumed. Paper off refuses a live venue.
 */
export function stopPaperImplementationShortfallParent(input: {
  parentClientOrderId?: string;
  kind?: string;
  status?: string;
  remaining?: string | null;
  paper?: PaperGate;
}): OmsIsPaperStopResult {
