...
**`dev` is frictionless, `staging`/`prod` are explicit.** With nothing configured, `dev`/`test` fall back to `http://localhost:3000` / `http://127.0.0.1:3000` / `:3100` (our own front-ends, both loopback spellings). `staging` and `prod` get no default. **DELETE THE :3000 PAIR when `apps/web` is deleted** — those two entries have no other caller, and left behind they hand a cross-origin grant to whatever a developer next starts on the most commonly squatted port on a workstation. They are still live today because `apps/web` still runs and is still bound to `3000:3000` in `docker-compose.apps.yml`; see the `DEV_ORIGINS` note in `src/cors.ts`.
