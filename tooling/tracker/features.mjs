...
export const FEATURES = [
  ...
  f('pay.rails', 'RailAdapter interface + crypto-native + card-sandbox', {
    ...
    note: 'Updated 2026-07-31: LIVE crypto rail exists. `EvmLiveChain` implements CryptoChainPort with posture:live (viem Public+Wallet client, HD acceptance addresses from PAY_CRYPTO_DEPOSIT_MNEMONIC, hot-wallet outbound via PAY_CRYPTO_HOT_WALLET_KEY, asset map PAY_CRYPTO_ASSETS, idempotent BroadcastStore). defaultChainFor builds it when PAY_CRYPTO_RPC_URL(+keys) are set; otherwise UnconfiguredChain in staging/prod and MemoryChain in dev/test. In-process CryptoChainWatcher POSTs signed webhooks. staging/prod omit card-sandbox by default (shouldRegisterCardSandbox) so boot posture passes with only live crypto. Proven: unit tests + optional anvil suite (evm-chain.live.test.ts). Residuals (honest, not blockers for this row): MemoryBroadcastStore is single-instance; production chain/RPC/custody are owner-supplied (anvil is not a chain decision); card acquiring remains a §13 commercial socket.',
  }),
