...
/**
 * HTTP door for caller min notional.
 * Missing notional when requested refuses. No invented last.
 */

const SECRET = 'matching-min-notional-router-secret-32';
...
describe('POST /markets/:marketId/orders min notional', () => {
  it('missing minNotional is a normal order — no invented floor', async () => {
  ...
  it('minNotional without a price refuses — last on the book is not a notional', async () => {
    ...
    const res = await post(app, submitBody({ type: 'market', price: null, tif: 'IOC', minNotional: '1000' }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(false);
    expect(res.json().rejected.code).toBe('missing_notional');
    expect(res.json().fills).toEqual([]);
    expect(res.json().resting).toBeNull();
    expect(engine.book(MARKET).toState().asks).toHaveLength(1);
    expect(engine.book(MARKET).toState().lastTradePrice).toBe('100');
    await app.close();
  });

  it('submit below the caller floor refuses', async () => {
    const { app, engine } = await mount();
    const res = await post(app, submitBody({ minNotional: '1000', qty: '9', price: '100' }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(false);
    expect(res.json().rejected.code).toBe('below_min_notional');
    expect(res.json().fills).toEqual([]);
    expect(engine.book(MARKET).toState().bids).toEqual([]);
    await app.close();
  });

  it('submit at the caller floor takes', async () => {
    const { app } = await mount();
    await post(
      app,
      submitBody({
    ...
    const res = await post(app, submitBody({ minNotional: '1000', qty: '10' }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(true);
    expect(res.json().fills[0].qty).toBe('2');
    await app.close();
