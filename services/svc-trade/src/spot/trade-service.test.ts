...
if (!available) {
  ...
  describe('cancel before fill', () => {
    it('releases the hold in full', async () => {
      ...
      expect((await trade.orderHistory(principalFor(ALICE), { tif: 'GTC' })).map((row) => row.id)).toEqual([order.id]);
    ...
    it('openOrders default still lists pending; status=open hides it', async () => {
      ...
      expect((await trade.openOrders(alice, undefined, undefined, 'buy')).map((row) => row.id).sort()).toEqual(
        [live.id, heldPending.id].sort(),
      );
      expect(await trade.openOrders(alice, undefined, undefined, 'sell')).toEqual([]);
      expect((await trade.openOrders(alice, undefined, undefined, undefined, 'limit')).map((row) => row.id).sort()).toEqual(
        [live.id, heldPending.id].sort(),
      );
      expect(await trade.openOrders(alice, undefined, undefined, undefined, 'market')).toEqual([]);
      expect((await trade.openOrders(alice, undefined, undefined, undefined, undefined, 'GTC')).map((row) => row.id).sort()).toEqual(
        [live.id, heldPending.id].sort(),
      );
  ...
  describe('refusals', () => {
    ...
    it('markets default still lists every kind; kind=futures hides spot', async () => {
      expect((await trade.markets()).some((row) => row.id === btcusdt.id && row.kind === 'spot')).toBe(true);
      expect((await trade.markets(undefined, 'futures')).some((row) => row.id === btcusdt.id)).toBe(false);
      expect((await trade.markets(undefined, 'spot')).map((row) => row.id)).toContain(btcusdt.id);
    });

    it('markets default still lists every quote; quote=BTC hides USDT spot', async () => {
      expect((await trade.markets()).some((row) => row.id === btcusdt.id && row.quoteAsset === 'USDT')).toBe(true);
      expect((await trade.markets(undefined, undefined, 'BTC')).some((row) => row.id === btcusdt.id)).toBe(false);
