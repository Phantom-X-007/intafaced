...
export interface PrivateRestDeps {
  ...
  listPositions(principal: Principal, symbol?: string, status?: 'open' | 'closing', side?: 'long' | 'short'): Promise<Position[]>;
...
export function parseOpenOrderStatus(raw: unknown): { ok: true; status?: 'pending' | 'open' } | { ok: false; message: string } {
  ...
  if (raw === 'pending' || raw === 'open') return { ok: true, status: raw };
...
export function parseOpenOrderClientId(raw: unknown): { ok: true; clientOrderId?: string } | { ok: false; message: string } {
  ...
  const clientOrderId = raw.trim();
  if (clientOrderId.length < 1 || clientOrderId.length > 64) {
    return { ok: false, message: 'clientOrderId must be 1-64 characters' };
  }
...
/** Optional live-position side filter. Absent → both long and short. */
export function parseOpenPositionSide(raw: unknown): { ok: true; side?: 'long' | 'short' } | { ok: false; message: string } {
  if (raw === undefined || raw === null || raw === '') return { ok: true, side: undefined };
  if (raw === 'long' || raw === 'short') return { ok: true, side: raw };
...
export function registerPrivateRest(app: FastifyInstance, deps: PrivateRestDeps): void {
  ...
  app.get<{ Querystring: { symbol?: string; status?: string; side?: string; type?: string; tif?: string; clientOrderId?: string } }>(
    '/api/v1/orders/open',
    async (req, reply) => {
      ...
      if (!clientIdParsed.ok) {
        return sendCcxt(reply, badRequest(clientIdParsed.message, 'trade.invalid_open_order_client_id'));
      }

      try {
        const orders = await deps.openOrders(
          principal,
          marketId,
          statusParsed.status,
          sideParsed.side,
   ...
   * ?status=open|closing and ?side=long|short filter in SQL.
   */
  app.get<{ Querystring: { symbol?: string; status?: string; side?: string } }>('/api/v1/positions', async (req, reply) => {
    const principal = requirePrincipal(req, reply);
    if (!principal) return;
    ...
    if (!sideParsed.ok) {
      return sendCcxt(reply, badRequest(sideParsed.message, 'trade.invalid_open_position_side'));
    }

    try {
      requireScope(principal, 'trade:read');
      const rows = await deps.listPositions(principal, req.query.symbol, statusParsed.status, sideParsed.side);
