...
describe('L3 wave130 support category catalog honesty', () => {
  it('category catalog boards', () => {
    ...
    expect(supportCategoryCatalogStatusLine()).toBe(
      'categories=4 deposit_withdraw=1 other=1',
    );
    expect(supportCategoryCatalogStatusLineMatches()).toBe(true);
    expect(supportCategoryCatalogStatusLineConsistent(supportCategoryCatalogStatusLine())).toBe(
      true,
    );
    expect(supportCategoryCatalogExportText().startsWith(supportCategoryCatalogExportHeader())).toBe(
      true,
    );
