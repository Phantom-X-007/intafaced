/**
 * Residual-release leftover already on an expired paper bracket parent.
 * Hands the retained leftover through ledger-client. This door never invents an
 * amount from a leg trigger or the clock, never invents a live venue, and does
 * not touch matching.
 */
import { formatAmount, parseAmount } from '@intafaced/ledger-client';
import type { PaperGate } from './oms-paper.js';

export type OmsPaperBracketReleaseResidualRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'not_expired'
  | 'already_released'
...
export type OmsPaperBracketReleaseResidualRefusal = {
  readonly ok: false;
  readonly reason: OmsPaperBracketReleaseResidualRefuseReason;
  readonly detail: string;
};

export type OmsPaperBracketReleaseResidualOk = {
  readonly ok: true;
...
export type OmsPaperBracketReleaseResidualResult =
  | OmsPaperBracketReleaseResidualOk
  | OmsPaperBracketReleaseResidualRefusal;

function refuse(
  reason: OmsPaperBracketReleaseResidualRefuseReason,
  detail: string,
): OmsPaperBracketReleaseResidualRefusal {
...
): { ok: true; text: string } | OmsPaperBracketReleaseResidualRefusal {
  if (raw === null || raw === undefined) {
    return refuse(
      'missing_residual',
      'residual.remaining is missing — refusing to invent leftover from a leg trigger or the clock',
    );
  }
  const text = raw.trim();
  if (text.length === 0) {
...
/**
 * Residual-release leftover already retained on an expired paper bracket parent through ledger-client.
 */
export function releaseExpiredPaperBracketResidual(input: {
  ...
  paper?: PaperGate;
}): OmsPaperBracketReleaseResidualResult {
