...
Two code-level contradictions are material. First, `planOmsArbAtomicLegs` reports `atomic: true` while `executeOmsArbAtomicLegs` explicitly submits sequentially and reports partial executions; it is a linked legged workflow, not venue-atomic execution. Second, the general OMS executor generates `oms-<venueId>` child IDs, which are not unique across distinct parent requests, and a later-leg `submit_failed` response does not carry already completed executions even though they may be journaled. This docs-only mission does not alter service code; the contract below makes the required truth and recovery authoritative, and implementation must correct those artifacts before relying on the paths professionally.
...
| `planOmsArbAtomicLegs` atomic label        | Contradiction: current execution is sequential and admits partial success. Canonical truth is `LEGGED`; no customer/product surface may claim atomic until a boundary proves all-or-none. Code/type naming is an implementation gap.  |
