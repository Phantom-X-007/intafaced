...
export const FEATURES = [
  ...
  f('trade.ccxt-api', 'CCXT-compatible public API (bots + terminals connect)', {
    ...
    phase: '2',
    dependsOn: ['trade.spot'],
    note: 'partial — public REST: markets, orderbook, ticker, tickers, trades (tape; ?since= ms), ohlcv (route exists, always [] until candle aggregation job — no inventing candles; ohlcv empty); private REST (edge-signed principal, fail-closed): GET orders/open|closed (?since= ms on closed), GET orders/:id, POST orders (placeOrder money path, trade:write + jurisdiction), DELETE orders/:id (cancelOrder), DELETE orders[?symbol=] (cancelAllOrders, sequential money path), GET account/trades (myFills; ?symbol= filter + ?since= ms), GET account/fees (published maker/taker bps per symbol; {} when none), GET account/balance (ledger projection, real self-only balances — not stub), GET positions (positions empty [] until trade.futures; setLeverage/setMarginMode not mounted). Still open: OHLCV empty (no candle job), futures leverage/margin when trade.futures exists. Private WS is under `ws.gateway` (/private/stream), not this REST surface. LIVE PROBE 2026-07-30 (fleet on main, through svc-edge :4000) contradicts the orderbook/ticker half of the first sentence: markets 200, tickers 200, trades 200, ohlcv 200 [], but `GET /api/v1/orderbook/:symbol` and `GET /api/v1/ticker/:symbol` return **502 MatchingUnavailable for every symbol svc-trade lists**. Two causes, and only the first is code: (1) svc-matching answers 404 MarketNotFound for a market it has never journalled, and svc-trade translates that into a 502 — claiming the engine is unavailable when the engine is up and answering (svc-ws, given the same 404, returns an honest 404 MarketNotFound; the unmerged `fix/spine-empty-book-not-502` is the fix and is not on main); (2) svc-matching derives its market set from journal replay only, so a book exists only after an order has been submitted — until trade.mm-bot seeds books, every market is bookless by construction. A bot reading this surface sees "exchange down" rather than "empty book".',
  }),
  ...
  f('pay.gateway', 'Branded gateway, hosted checkout, payment links', {
    ...
    note: 'Updated 2026-07-30: payment links + minimal hosted page. createLink / resolveLink / listLinks / deactivateLink on tRPC; GET /checkout?token= and /pay/link/:token return HTML (label, amount, currency; honest expired/not-found; no card capture — CTA completes in merchant app). Edge: /api/pay/checkout?token=. Rails still sandbox (MemoryChain + card-sandbox).',
  }),
