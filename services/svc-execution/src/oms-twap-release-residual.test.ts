...
import { releaseExpiredTwapResidual } from './oms-twap-release-residual.js';
...
describe('releaseExpiredTwapResidual', () => {
  it('refuses missing / whitespace parentClientOrderId', () => {
    expect(
      releaseExpiredTwapResidual({
        kind: 'twap',
        status: 'expired',
        remaining: REMAINING,
        jobs: JOBS_ON,
  ...
  it('happy: expired + remaining — residual released via ledger-client, not paper', () => {
    const result = releaseExpiredTwapResidual({
      parentClientOrderId: 'p-twap',
      kind: 'twap',
      status: 'expired',
      remaining: REMAINING,
