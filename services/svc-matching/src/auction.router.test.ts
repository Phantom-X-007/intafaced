...
/**
 * HTTP door for auction / benchmark.
 * Unsupported intent refuses. Missing or false is a normal order. No invented auction price.
 */

const SECRET = 'matching-auction-router-secret-32chars';
...
describe('POST /markets/:marketId/orders auction / benchmark', () => {
  it('missing flags are a normal order — no invented auction price', async () => {
  ...
  it('auction:true refuses — no rest as a limit', async () => {
    const { app, engine } = await mount();
    const res = await post(app, submitBody({ auction: true }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(false);
    expect(res.json().rejected.code).toBe('auction_unsupported');
  ...
  it('benchmark:true refuses — no invented benchmark price', async () => {
    const { app, engine } = await mount();
    const res = await post(app, submitBody({ benchmark: true }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(false);
    expect(res.json().rejected.code).toBe('benchmark_unsupported');
    expect(res.json().fills).toEqual([]);
    expect(engine.book(MARKET).toState().bids).toEqual([]);
    await app.close();
  });

  it('auction:false / benchmark:false is a normal order', async () => {
    const { app } = await mount();
    await post(
      app,
      submitBody({
    ...
    const res = await post(app, submitBody({ auction: false, benchmark: false }));
    expect(res.statusCode).toBe(200);
    expect(res.json().accepted).toBe(true);
    expect(res.json().fills[0].qty).toBe('2');
    await app.close();
