...
GitHub Actions jobs failed with: *“recent account payments have failed or your spending limit needs to be increased.”*  
...
## Free mountains still on the table (honest)

- `trade.convert` / `protocol.amm` — code on main but tracker correctly **wip** (money-suite / chain deploy gaps)  
- `ws.gateway` — depth + trade tape; orders/positions still missing  
- `pay.gateway` — mount yes; hosted checkout/payment links no  
- `protocol.smart-accounts` — high leverage, needs non-propped chain path  
- `ops.notifications` / `infra.i18n` — large product surfaces  
