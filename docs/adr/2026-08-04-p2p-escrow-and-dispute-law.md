...
## Decision 2 — the backstop is a timeout disposition, not a resolution
...
> **The backstop stays. It is renamed to what it is — a timeout disposition — and it may not fire while the moderation path is unreachable.**

The spec's principle is upheld by fixing the real defect, which is not the timer. **It is that a human cannot intervene even if they want to:**
...
**A timer that fires because nobody could have looked is not a fallback. It is the only path.** That is what makes the current behaviour a violation; the timer itself is not.

---

## The rule

**A dispute may only time out if a human could have reached it.** Concretely, the backstop is gated on the moderation path being reachable: a queue that lists open disputes, evidence a moderator can read, and a scope that can actually be held. Until those exist, a dispute reaching its deadline **escalates and holds** rather than disposing of value.

**And the disposition is disclosed before it can happen.** A party entering a dispute is told, at that moment, what happens if nobody rules — including which way it defaults. The current default routes the loss off-platform on the reasoning that _"refunding a buyer who did pay leaves them a fiat claim they can still pursue through their bank."_ That may be the right call; it is currently made by an env default with no product doc behind it, and a user is never told.
...
> **Close the oracle, do not replace it.** The refusal must not distinguish "no such instrument" from any other reason the take could not proceed, and it must be access-logged like every other read of instrument existence.
...
> **The caps are not the control. The operator boundary is.** Say so where the caps are defined, so nobody later reasons "it is capped, therefore it is safe."
...
> **This is tracked as a defect, not recorded as a §13 socket.** Nothing is missing from the world; only code is missing.
...
- **The default disposition** when nobody rules — refund vs release. It is currently an env default with no product decision behind it.
- Whether `admin:compliance` becomes holdable, and by whom.
- The P2P erase path's scope, which is a legal question before it is a technical one.
