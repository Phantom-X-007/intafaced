 ...
 * Migration runner.
 *
 * Applies every `NNNN_*.sql` in order, tracked in `ledger.__migrations` so a
 ...
 * should run it against a book with real transactions in it.
 */
 ...
 * strays outside `ledger` fails here rather than in production.
 */
const url = process.env.DATABASE_URL ?? 'postgres://svc_ledger:svc_ledger@localhost:5433/intafaced';
...
async function main() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS ledger.__migrations (
  ...
  if (down) {
    for (const up of [...ups].reverse()) {
      ...
      await sql`DELETE FROM ledger.__migrations WHERE name = ${up}`;
    }
    console.log('✓ reversed');
    return;
  }

  const applied = new Set((await sql<Array<{ name: string }>>`SELECT name FROM ledger.__migrations`).map((r) => r.name));
  ...
  for (const file of ups) {
    ...
    await sql`INSERT INTO ledger.__migrations (name) VALUES (${file})`;
  }
