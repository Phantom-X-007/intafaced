...
1. **Inventory PR (docs/tooling):** script lists controllers + `MemberWalletDao` mutators under `vendor/coinexchange/**` (paths already include `MemberWalletController`, finance controllers, etc.).  
...
5. **Futures REST on tip (#278 era)** does not reorder Spec GC-5 for *this* program’s residual board (seed honesty + dual-book + CEX chaos remain first).
...
| Q | Default |
| --- | --- |
| Exact Spring door mechanism | URL security filter fail-closed; inventory-driven allowlist of *non*-money routes only if needed |
