...
**43 of 108 shipped (40%)** · 2 in progress · 31 ready to claim · 32 blocked · 19 deliberate §13 sockets
...
| RailAdapter interface + crypto-native + card-sandbox | `pay` | 3 | `pay.rails` |
...
| **7** | RailAdapter interface + crypto-native + card-sandbox | 🟢 ready | `pay.rails` |
...
| 🔨 | WebSocket fan-out: depth, trades, orders, positions <br/>_Still wip on main. Depth done + public TRADE tape + private orders/fills on /private/stream (orderUpdated+fillSettled, JWT). Fourth stream (positions / positionUpdated) is only on open PR #227 — not merged; title names four streams; three of four is not done._ | F |  | `ws.gateway` |
...
| 🟢 | RailAdapter interface + crypto-native + card-sandbox <br/>_Updated 2026-07-31 honesty re-pass: still `ready` not `done` on main. Interface + conformance kit are real and tested; NEITHER v1 RAIL ON MAIN MOVES REAL VALUE and the code now says so. Every adapter declares `mode: live|sandbox` (conformance kit refuses one that does not); crypto-native derives its mode from the chain port, so MemoryChain makes it a sandbox whatever §13 says. Two gates in rails/posture.ts: APP_ENV staging/prod REFUSES TO BOOT with any sandbox rail registered unless PAY_ALLOW_SANDBOX_RAILS=true, and payout/refund re-check at the call site before the ledger moves (pay.rail_not_live -> SERVICE_UNAVAILABLE). Production chain default is UnconfiguredChain, which refuses every call instead of returning a fabricated txHash. /ready and railHealth carry mode. Live EVM crypto rail (EvmLiveChain, watcher, HD accept, hot-wallet payout) exists only on open PR #226 — not merged; do not mark rails done from that PR until it lands._ | F |  | `pay.rails` |
| ⛔ | User deposit + withdrawal — the two paths off the merchant path <br/>_Updated 2026-07-31 honesty re-pass: still not `done` on main. deposit.credit (admin:treasury) and withdrawal.* (trade:withdraw INTERACTIVE_ONLY + 2FA / trade:read) are reachable on mounted /trpc via svc-edge /api/pay — edge-signed principal required (router.mount.test.ts). Double-submit proven under CONCURRENCY: two identical withdrawals in flight debit once and carry ONE rail idempotency key; a redelivered deposit credits once; two concurrent affordable-alone withdrawals cannot overdraw. Money-path suite against real Postgres on intafaced_test. Depends on pay.rails, which has no live rail on main (#226 open only) — a sandbox payout is refused before anything moves rather than fabricating a `sent`._ | F | `pay.rails` | `pay.user-money` |
