...
describe('L3 wave138 room access kind honesty', () => {
  it('access kind catalog and stake board', () => {
    ...
    expect(roomAccessKindCatalogStatusLineConsistent(roomAccessKindCatalogStatusLine())).toBe(
      true,
    );
    expect(roomAccessKindCatalogExportText().startsWith(roomAccessKindCatalogExportHeader())).toBe(
      true,
    );
