...
**Must not:** constructor or init params that invent `house_fee_bps`, allocation %, vest cliff/duration, hard/soft cap, or a second tier table. A contract that *accepts* owner-supplied params at deploy is fine **only if** the product refuses to build calldata until the authority store is populated. Hard-coded “temporary” 5% / 10% / 30-day vest is a doctrine crime on the chain plane the same as on Fiat.
