...
4. Therefore: **there is no URL on `:8090` that reaches the depth stream** (REGROUP §3). Upgrade headers on `/ws` are correct for *a* WS proxy, but the path never matches the service contract.
...
- [ ] **Proof on fleet:** intersection script + one live snapshot frame through `:8090` (acceptance from hard board: *“Live shell can open depth for a real market id”*).
