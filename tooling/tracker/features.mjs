...
export const FEATURES = [
  ...
  f('trade.mm-bot', 'Internal market-maker seeding books at launch', {
    ...
    note: '2026-08-14 orderFilled recovery: omitted / HOUSE_MM_USER_UUID makerAccountId on a recorded seed row rewrites to house:market-maker (never an anonymous customer-looking fill); empty + no house row still refuses. 2026-08-14 production mid: TRADE_MM_SEED_MID_FROM_VENUE + TRADE_VENUE_MARK_STREAM uses the same MaintainedBook port as futures marks (desynced → skip seed, never invent). Owner released 2026-08-08 (axis C1 / Nitro green light). seedMarket + job OFF default + marketMakerMakerFill + settleFill house-MM; cancel/reseed lifecycle + mid port on main (MM-1/2/3). D26-P1-T10 backend honesty: seed-honesty contract (flagged / killable / no manufactured crosses); resting seeds recorded seeded=true; TRADE_MM_SEED_ENABLED kills placeOrder seeded path too. Residual: production mid ops. Not Done — ready with ops kill-switches.',
  }),
