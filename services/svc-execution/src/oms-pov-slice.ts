/**
 * Slice one child of a live POV parent.
 * Qty, credit, and residual leftover are ledger amounts. Missing/blank qty
 * refuses — never invent size from participation or duration. Missing/blank
 * credit refuses. Residual leftover is never invented from participation,
 * duration, or credit. Not paper. Does not submit to matching.
 */
...
export type OmsPovSliceRefuseReason =
  | 'missing_parent'
  | 'not_live'
  | 'not_running'
  | 'missing_qty'
...
export type OmsPovSliceRefusal = {
  readonly ok: false;
  readonly reason: OmsPovSliceRefuseReason;
  readonly detail: string;
};

export type OmsPovSliceOk = {
  readonly ok: true;
...
export type OmsPovSliceResult = OmsPovSliceOk | OmsPovSliceRefusal;

function refuse(reason: OmsPovSliceRefuseReason, detail: string): OmsPovSliceRefusal {
  return { ok: false, reason, detail };
}

function parseQty(raw: string | null | undefined): { ok: true; text: string } | OmsPovSliceRefusal {
  if (raw === null || raw === undefined) {
    return refuse('missing_qty', 'slice qty is blank — refuse rather than invent size from participation or duration');
  }
  const text = raw.trim();
  if (text.length === 0) {
    return refuse('missing_qty', 'slice qty is blank — refuse rather than invent size from participation or duration');
  ...
  try {
    ...
    if (value <= 0n) {
      return refuse('qty_invalid', 'slice qty must be a positive ledger amount — not invented from participation or duration');
    }
...
function parseCredit(raw: string | null | undefined): { ok: true; text: string } | OmsPovSliceRefusal {
  if (raw === null || raw === undefined) {
...
function parseRetainedRemaining(raw: string | null | undefined): { ok: true; text: string } | OmsPovSliceRefusal {
  if (raw === null || raw === undefined) {
    return refuse(
      'missing_residual',
      'residual.remaining is missing — refusing to invent leftover from participation, duration, or credit',
    );
  }
  const text = raw.trim();
  if (text.length === 0) {
...
/**
 * Slice one child of a live POV parent using caller qty.
 * Participation and duration are ignored for size — never a substitute leftover.
 * Jobs off refuses. Blank credit refuses. Not paper.
 */
export function slicePovParent(input: {
  parentClientOrderId?: string;
  kind?: string;
  status?: string;
  amount?: string | null;
  /** Retained max participation from live start. Must not be used as qty. */
  ...
  if (input.status !== 'running') {
    return refuse('not_running', `parent ${parentClientOrderId} is ${input.status ?? 'not running'} — slice needs a running POV parent`);
  }
