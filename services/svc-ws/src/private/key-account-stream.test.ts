...
describe('private stream drops when the API key is bound to another account', () => {
  let server: Server;
  ...
  it('session token (no kid) is not account-gated', async () => {
    const { host, port } = await boot(keyPort(ACC));
    const issued = await issueAccessToken({ userId: USER, sessionId: SESSION, scopes: ['trade:write'] }, tokens);
    expect(await upgradeStatus(host, port, `${PRIVATE_STREAM_PATH}?access_token=${issued.token}`, OTHER)).toBe(101);
  });
